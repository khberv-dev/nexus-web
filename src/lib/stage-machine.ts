import { StageStatus, OrderStatus, Role, PaymentStatus } from "@prisma/client";
...
export async function activateNextStage(orderId: string, currentType: string): Promise<void> {
  ...
  if (await isOrderComplete(orderId)) {
    ...
    try {
      const order = await prisma.order.findUnique({ where: { id: orderId }, select: { clientId: true, specialistId: true } });
      if (order) {
