...
const initialData = {
  columns: {
    "todo": { id: "todo", title: "К выполнению", cardIds: ["c1", "c2", "c3", "c4", "c5", "c6", "c7", "c8", "c9", "c10", "c11", "c12", "c13"] },
  ...
  cards: {
    ...
    "c2": {
      id: "c2", title: "Аутентификация и роли (Zitadel)", tag: "Auth", tagColor: "#00cfe8", hours: 28, avatars: ["Б"],
    ...
    "c5": {
      id: "c5", title: "Регистрация заказчика и бриф", tag: "Клиент", tagColor: "#ea5455", hours: 24, avatars: ["Б"],
    ...
    "c6": {
      id: "c6", title: "Стейт-машина проекта (3 этапа)", tag: "Ядро", tagColor: "#ea5455", hours: 40, avatars: ["А", "Б"],
    ...
    "c7": {
      id: "c7", title: "Загрузка и хранение файлов (S3)", tag: "S3", tagColor: "#00cfe8", hours: 16, avatars: ["А"],
    ...
    "c10": {
      id: "c10", title: "Административная панель", tag: "Админ", tagColor: "#ff9f43", hours: 32, avatars: ["А", "В"],
    ...
    "c11": {
      id: "c11", title: "Личный кабинет специалиста", tag: "Специалист", tagColor: "#7367f0", hours: 20, avatars: ["Б"],
    ...
    "c12": {
      id: "c12", title: "Личный кабинет заказчика", tag: "Заказчик", tagColor: "#00cfe8", hours: 16, avatars: ["В"],
        ...
        <div className="modal-dialog modal-dialog-centered modal-lg" onClick={e => e.stopPropagation()}>
          <div className="modal-content">
            <div className="modal-header">
              <div>
                <span className="kanban-tag me-2" style={{ backgroundColor: card.tagColor + "22", color: card.tagColor }}>{card.tag}</span>
                ...
                <div>
                  ...
                  <div className="kanban-avatars mt-1">
                    {card.avatars.map((letter, i) => (
                      <span key={i} className="kanban-avatar" style={{ backgroundColor: AVATAR_COLORS[i % AVATAR_COLORS.length] }}>{letter}</span>
                    ))}
                  </div>
                </div>
              </div>
...
function KanbanCard({ card, isDragging, onClick }: { card: Card; isDragging?: boolean; onClick?: () => void }) {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <span className="kanban-tag" style={{ backgroundColor: card.tagColor + "22", color: card.tagColor }}>{card.tag}</span>
...
function KanbanColumn({ column, cards, onCardClick }: { column: { id: string; title: string; cardIds: string[] }; cards: Card[]; onCardClick: (card: Card) => void }) {
  const { setNodeRef } = useDroppable({ id: column.id })
  const totalHours = cards.reduce((s, c) => s + c.hours, 0)
  return (
    <div className="kanban-column">
      <div className="kanban-column-header">
        <h3>{column.title} <span style={{ fontSize: 12, color: "#999", fontWeight: 400 }}>({cards.length})</span></h3>
        {totalHours > 0 && <span style={{ fontSize: 11, color: "#7367f0", fontWeight: 600 }}>{totalHours} ч</span>}
      </div>
...
export default function MvpKanban() {
  ...
  const handleDragOver = ({ active, over }: DragOverEvent) => {
    ...
    setData(prev => {
      ...
      return { ...prev, columns: { ...prev.columns, [fromCol]: { ...prev.columns[fromCol], cardIds: src }, [toCol]: { ...prev.columns[toCol], cardIds: dst } } }
    })
  }
  ...
  const handleDragEnd = ({ active, over }: DragEndEvent) => {
    ...
    if (oldIdx !== newIdx && newIdx !== -1)
      setData(prev => ({ ...prev, columns: { ...prev.columns, [colId]: { ...prev.columns[colId], cardIds: arrayMove(cards, oldIdx, newIdx) } } }))
  }
    ...
    <>
      <div className="card mb-4">
        <div className="card-body">
          ...
          <div className="progress" style={{ height: 8 }}>
            <div className="progress-bar bg-primary" style={{ width: `${Math.round(doneHours / totalHours * 100)}%` }} />
          </div>
          <div className="d-flex gap-3 mt-2">
            {data.columnOrder.map(colId => {
              const col = data.columns[colId]
              const colors: Record<string, string> = { "todo": "secondary", "in-progress": "warning", "review": "info", "done": "success" }
              return <span key={colId} className={`badge bg-label-${colors[colId]}`}>{col.title}: {col.cardIds.length}</span>
            })}
          </div>
        </div>
      </div>
      ...
      <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
        <div className="kanban-board">
          {data.columnOrder.map(colId => (
            <KanbanColumn key={colId} column={data.columns[colId]} cards={data.columns[colId].cardIds.map(id => data.cards[id])} onCardClick={setModalCard} />
          ))}
        </div>
