        ...
        >
          {countries.map(c => (
            <div key={c} data-country={c}>
              <CountryOption country={c} selected={c === value} onClick={() => { onChange(c); close() }} />
            </div>
          ))}
          {countries.length === 0 && (
            <p style={{ color: "rgba(255,255,255,0.3)", fontSize: "0.8rem", textAlign: "center", padding: 16 }}>Не найдено</p>
          )}
        </div>
      </div>,
      document.body
