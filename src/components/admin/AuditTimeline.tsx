...
const ACTION_LABEL: Record<string, { label: string; icon: string; color: string }> = {
  ...
  framework_contract_admin_signed: { label: "Договор оказания услуг подтвержден", icon: "bx-check-shield", color: "#22c55e" },
...
export function AuditTimeline({ entity, entityId }: { entity: string; entityId: string }) {
  ...
  if (logs.length === 0) return <div style={{ padding: 12, fontSize: "0.75rem", color: "var(--adm-muted)" }}>Нет записей</div>

  return (
    <div style={{ position: "relative", paddingLeft: 20 }}>
      <div style={{ position: "absolute", left: 6, top: 4, bottom: 4, width: 1, background: "var(--adm-sidebar-border, rgba(255,255,255,0.08))" }} />

      {logs.map((log, i) => {
        const meta = ACTION_LABEL[log.action] ?? { label: log.action.replace(/_/g, " "), icon: "bx-dots-horizontal-rounded", color: "var(--adm-muted)" }
          ...
          <div key={log.id} style={{ position: "relative", paddingBottom: i < logs.length - 1 ? 14 : 0 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
              <div style={{ minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: "0.78rem", fontWeight: 500 }}>
              ...
              <div style={{ fontSize: "0.65rem", color: "var(--adm-muted)", whiteSpace: "nowrap", textAlign: "right" }}>
                <div>{day}</div>
                <div>{time}</div>
              </div>
            </div>
            ...
            {changes.length > 0 && (
              <div style={{ marginTop: 4, paddingLeft: 2 }}>
                {changes.slice(0, 6).map(([k, v]) => {
                  ...
                  return (
                    <div key={k} style={{ fontSize: "0.67rem", color: "var(--adm-muted)", lineHeight: 1.5 }}>
                      <span style={{ color: "var(--adm-text, #f1f5f9)" }}>{label}</span>{": "}
                      {v.from != null && <><span style={{ textDecoration: "line-through", opacity: 0.4 }}>{from}</span> → </>}
                      <span style={{ color: "#22c55e" }}>{to}</span>
                    </div>
                  )
                })}
                {changes.length > 6 && <div style={{ fontSize: "0.65rem", color: "var(--adm-muted)", opacity: 0.5 }}>+{changes.length - 6} еще</div>}
              </div>
            )}
          </div>
        )
