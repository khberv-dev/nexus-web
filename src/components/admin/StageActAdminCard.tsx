...
import type { Stage, StageAct, ActStatus } from "@/app/admin/orders/types"
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {act.status === "SPECIALIST_UPLOADED" && (
            <p style={{ fontSize: "0.72rem", color: "var(--adm-muted)", margin: 0, lineHeight: 1.35 }}>
              «Одобрить» открывает заказчику скачивание акта и загрузку подписанного PDF. «Подтвердить» (финально) — только после того, как заказчик загрузит подпись.
            </p>
          )}
          ...
          {act.status === "SPECIALIST_UPLOADED" && (
            <>
              <button onClick={() => void handleApprove()} disabled={acting} className="sp-btn sp-btn-success sp-btn-sm">
                {acting ? "…" : "Одобрить"}
              </button>
              <button
                onClick={() => {
          ...
          {act.status === "CLIENT_SIGNED" && (
            <button onClick={() => void handleConfirm()} disabled={acting} className="sp-btn sp-btn-success sp-btn-sm">
              {acting ? "…" : "Подтвердить"}
            </button>
          )}
          </div>
      ...
      {rejecting && (
        ...
        >
          ...
          >
            ...
            <p style={{ color: "#999", fontSize: "0.85rem", marginBottom: 16 }}>Укажите причину возврата акта на доработку</p>
            <textarea
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <button type="button" onClick={() => setRejecting(false)} disabled={acting} className="sp-btn sp-btn-ghost">
                Отмена
              </button>
              <button
                type="button"
