...
import type { AnnotoriousImageAnnotator, ImageAnnotation, PopupProps } from "@annotorious/react"
...
export function CommentPopup(props: CommentPopupProps) {
  ...
  return (
    <div style={commentPopupRoot}>
      <div style={{ fontSize: 11, fontWeight: 600, marginBottom: 6, color: "var(--dash-text, rgba(255,255,255,0.9))" }}>
      ...
      {canEdit ? (
        <>
          ...
          <p style={{ margin: "6px 0 0", fontSize: 10, color: "var(--dash-muted, rgba(255,255,255,0.62))", lineHeight: 1.4 }}>
            Чтобы отправить все пометки дизайнеру, внизу под картинкой нажмите «Сохранить пометки».
          </p>
        </>
      ) : (
        <p style={{ margin: "8px 0 0", fontSize: 11, color: "var(--dash-muted, rgba(255,255,255,0.62))", lineHeight: 1.45 }}>
          <strong style={{ color: "var(--dash-text, rgba(255,255,255,0.9))" }}>Только просмотр.</strong> Комментарии к областям может оставлять только
