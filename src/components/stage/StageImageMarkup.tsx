...
import { markupPopupZIndexCss, markupHintColor } from "./markup/constants"
...
export default function StageImageMarkup({ stageId, fileId, filename, editable, readonlyReason, onClose }: StageImageMarkupProps) {
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 10 }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: "0.85rem", fontWeight: 700, color: "var(--dash-text)" }}>Разметка изображения</div>
        </div>
        {onClose ? (
          <button
            type="button"
      ...
      {!editable && readonlyReason ? (
        ...
        >
          <strong style={{ color: "var(--dash-warn, #ff9f43)" }}>Разметка недоступна для редактирования.</strong>{" "}
          {readonlyReason}
        </div>
      ) : null}
