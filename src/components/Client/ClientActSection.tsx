...
export function ClientActSection({stage, onUploadSigned}: Props) {
    ...
    const getBgColor = () => {
        switch (act.status) {
            case "REJECTED":
                return "rgba(234,84,85,0.06)"
            case "CONFIRMED":
                return "rgba(46,184,92,0.06)"
            default:
                return "var(--dash-surface)"
        }
    }
    ...
    return (
        ...
        >
          ...
          Акт · {STAGE_LABEL[stage.type]}
                ...
                <span style={{
                    fontSize: "0.75rem",
                    color: act.status === "REJECTED" ? "var(--dash-danger)" : act.status === "CONFIRMED" ? "var(--dash-success)" : "var(--dash-warn)",
                    fontWeight: 500
                }}>
          {statusLabel}
        </span>
