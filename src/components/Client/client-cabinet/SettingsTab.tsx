...
export function SettingsTab({ name, email, formData }: { name: string; email: string; formData: Record<string, string> }) {
  ...
  return (
    <div>
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <p style={{ fontWeight: 600, fontSize: "0.95rem", color: "var(--dash-text)", margin: "0 0 2px" }}>{name || "—"}</p>
      ...
      <form onSubmit={handleSave} noValidate>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <DashSettingsSection title="Контактные данные" iconClass="bx bx-user" className="dash-surface-card--pad-md">
            <F label="ФИО">
              <input
                type="text"
            ...
            <F label="Телефон">
              <input type="tel" value={form.phone || ""} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} style={settingsInputStyle} />
            </F>
            <F label="Сайт">
              <input
                type="url"
            ...
            <F label="Город">
              <input type="text" value={form.city || ""} onChange={e => setForm(p => ({ ...p, city: e.target.value }))} style={settingsInputStyle} />
            </F>
            <F label="ЭДО">
              <p style={{ fontSize: "0.72rem", color: "var(--dash-muted)", margin: "0 0 8px", lineHeight: 1.4 }}>
                Электронный документооборот: Контур.Диадок, Такском, СБИС, 1С-ЭДО — отметьте, чем пользуетесь
              </p>
          ...
          <DashSettingsSection title="Реквизиты" iconClass="bx bx-building" className="dash-surface-card--pad-md">
            <F label="Правовая форма" required>
              <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
                {LEGAL_FORMS.map(c => (
                  <SettingsChip key={c} label={c} active={form.legalForm === c} onClick={() => setForm(f => ({ ...f, legalForm: c }))} />
                ))}
              </div>
