...
export function PaymentsTab({
  ...
  const stats = [
    { label: "Оплачено", value: totalPaid, icon: "bx-check-circle", color: "var(--dash-success)", bg: "var(--dash-success-bg)" },
    { label: "Удержано", value: totalHeld, icon: "bx-time-five", color: "var(--dash-warn)", bg: "var(--dash-warn-bg)" },
    { label: "К оплате", value: totalPending, icon: "bx-credit-card", color: "var(--dash-danger)", bg: "var(--dash-danger-bg)" },
  ].filter(s => s.value > 0)
      ...
      <DocSection title="Реквизиты" icon="bx-building">
        {!isComplete ? (
          <div style={{ padding: "0.25rem 0" }}>
            <p style={{ fontSize: "0.8rem", color: "var(--dash-danger)", fontWeight: 500, margin: "0 0 8px" }}>
              Реквизиты не заполнены или заполнены не полностью
            </p>
            {requisitesError && (
              <p style={{ fontSize: "0.75rem", color: "var(--dash-muted)", margin: "0 0 12px" }}>{requisitesError}</p>
            )}
            <button
            ...
            {contracts.map(c => {
              ...
              return (
                <tr key={c.id} style={{ borderBottom: "1px solid var(--dash-border)" }}>
                  ...
                  <td style={{ padding: "8px 0", color: "var(--dash-muted)" }}>#{c.orderId.slice(-6).toUpperCase()}</td>
                  ...
                  <td style={{ padding: "8px 0", textAlign: "right" }}>
                    {c.s3Key ? (
                      <DashActionLink href={`/api/files/download?key=${encodeURIComponent(c.s3Key)}`} iconClass="bx-download" native>
                        Скачать
                      </DashActionLink>
                    ) : (
                      "—"
            ...
            {invoices.map(inv => {
              ...
              return (
                <tr key={inv.id} style={{ borderBottom: "1px solid var(--dash-border)" }}>
                  ...
                  <td style={{ padding: "8px 0", textAlign: "right" }}>
                    {inv.s3Key ? (
                      <DashActionLink href={`/api/files/download?key=${encodeURIComponent(inv.s3Key)}`} iconClass="bx-download" native>
                        Скачать
                      </DashActionLink>
                    ) : (
                      "—"
              ...
              <div key={a.id} className="dash-finance-card">
                <div className="dash-finance-card__row">
                  <div className="dash-finance-card__meta">
                    <span className="dash-finance-card__title">{ACT_STAGE_LABEL[a.stageType] ?? a.stageType}</span>
                    <span className="dash-finance-card__sub">#{a.orderId.slice(-6).toUpperCase()}</span>
                  </div>
                  <StatusBadge variant={a.signedAt ? "done" : "current"} label={a.signedAt ? "Подписан" : "Ожидает подписи"} />
                </div>
            ...
            {payments.map(p => {
              ...
              return (
                <tr key={p.id} style={{ borderBottom: "1px solid var(--dash-border)" }}>
                  <td style={{ padding: "8px 0", fontWeight: 500 }}>#{p.order.id.slice(-6).toUpperCase()}</td>
                  <td style={{ padding: "8px 0", color: "var(--dash-muted)" }}>{brief?.objectType ?? "—"}</td>
                  <td style={{ padding: "8px 0", color: "var(--dash-muted)" }}>
