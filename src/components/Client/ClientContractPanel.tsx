      ...
      >
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          <button onClick={onClose} disabled={loading}
            style={{ padding: "8px 16px", borderRadius: 6, border: "1px solid #444", background: "#222", color: "#999", cursor: "pointer", fontSize: "0.85rem" }}>
            Отмена
          </button>
          <button onClick={handleSubmit} disabled={!file || loading}
            style={{ padding: "8px 16px", borderRadius: 6, border: "none", background: "#34d399", color: "#fff", cursor: !file || loading ? "not-allowed" : "pointer", fontSize: "0.85rem", fontWeight: 600 }}>
...
function ContractFileLink({ contractId, s3Key, label }: { contractId: string; s3Key: string | null; label: string }) {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      style={{ display: "inline-flex", alignItems: "center", gap: 4, color: "#34d399", fontSize: "0.85rem", textDecoration: "none" }}
    >
...
export function ClientContractPanel({ contract, orderId, userRole, onUploadSigned }: Props) {
  ...
  if (!contract) {
    return (
      <div style={{ background: "var(--dash-surface)", borderRadius: 10, padding: 16, border: "1px solid var(--dash-border)", marginBottom: 16 }}>
        <div style={{ fontSize: "0.75rem", color: "var(--dash-muted)", textTransform: "uppercase", marginBottom: 8 }}>Договор</div>
  ...
  return (
    <div style={{ background: "var(--dash-surface)", borderRadius: 10, padding: 16, border: "1px solid var(--dash-border)", marginBottom: 16 }}>
      <div style={{ fontSize: "0.75rem", color: "var(--dash-muted)", textTransform: "uppercase", marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span>Договор</span>
