    ...
    <>
      ...
      >
        {[
          ...
          <div key={item.label} style={{ minWidth: 100 }}>
            <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.7rem", textTransform: "uppercase", letterSpacing: "0.07em", margin: "0 0 4px" }}>
              {item.label}
            </p>
      ...
      {d.bio && (
        <div style={{ marginBottom: 24 }}>
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.7rem", textTransform: "uppercase", letterSpacing: "0.07em", margin: "0 0 10px" }}>
            О себе
          </p>
          <p style={{ color: "rgba(255,255,255,0.75)", fontSize: "0.9rem", lineHeight: 1.65, margin: 0 }}>{d.bio}</p>
        </div>
      )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: `repeat(${works.length}, 1fr)`, gap: 8 }}>
            {works.map((src, i) => (
              ...
              >
                ...
                <img src={src} alt={`Работа ${i + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
              </button>
            ))}
          </div>
        </div>
