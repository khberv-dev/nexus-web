...
export const StageChatPanel = forwardRef<StageChatPanelHandle, StageChatPanelProps>(function StageChatPanel(
  ...
  const load = useCallback(async () => {
    try {
      ...
      const payload = data && typeof data === "object" ? (data as { messages?: unknown; viewerId?: unknown }) : null
  ...
  return (
    ...
    >
      {!inDrawer ? (
        ...
        >
          <i className="bx bx-message-dots" style={{ color: "var(--dash-accent)", fontSize: "1rem" }} aria-hidden />
          Чат по этапу
        </div>
      ) : null}

      <p style={{ margin: 0, padding: inDrawer ? "10px 14px 0" : "8px 14px 0", fontSize: "0.72rem", color: "var(--dash-muted)", lineHeight: 1.45, flexShrink: 0 }}>
        {counterpartHint}
      </p>
          ...
          messages.map(m => {
            ...
            return (
              ...
              >
                ...
                <div style={{ fontSize: "0.82rem", color: "var(--dash-text)", whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
                  {m.body}
                </div>
              </div>
            )
      ...
      >
        ...
        <span style={{ fontSize: "0.65rem", color: "var(--dash-muted)", paddingBottom: inDrawer ? 2 : 0, display: "block" }}>
