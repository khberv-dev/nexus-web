...
const ACTION_LABEL: Record<string, { label: string; icon: string; color: string }> = {
  ...
  framework_contract_admin_signed: { label: "Рамочный договор подтверждён", icon: "bx-check-shield", color: "var(--dash-success)" },
  ...
  stage_file_annotations_saved: { label: "Пометки на изображении отправлены", icon: "bx-image", color: "var(--dash-accent)" },
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: "0.8rem", fontWeight: 600 }}>
                      <i className={`bx ${meta.icon}`} style={{ color: meta.color, fontSize: "0.9rem" }} />
                      {meta.label}
                    </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                    <div style={{ fontSize: "0.65rem", color: "var(--dash-muted)", whiteSpace: "nowrap", textAlign: "right" }}>
                      <div>{day}</div>
                      <div>{time}</div>
                    </div>
                    {jump}
                ...
                {changes.length > 0 && (
                  <div style={{ marginTop: 6 }}>
                    {changes.slice(0, 4).map(([k, v]) => {
                      ...
                      return (
                        <div key={k} style={{ fontSize: "0.68rem", color: "var(--dash-muted)", lineHeight: 1.45 }}>
                          <span style={{ color: "var(--dash-text2)" }}>{label}</span>
                          {": "}
                          {v.from != null && v.from !== "" && (
                            <>
                              <span style={{ textDecoration: "line-through", opacity: 0.45 }}>{from}</span>
                              {" → "}
                            </>
                          )}
                    ...
                    {changes.length > 4 && (
                      <div style={{ fontSize: "0.65rem", color: "var(--dash-muted)", opacity: 0.65 }}>
