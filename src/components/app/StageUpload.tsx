...
export function StageUpload({
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    e.preventDefault()
    setDragOver(false)
    const droppedFiles = Array.from(e.dataTransfer.files).map(f => ({ file: f, progress: 0, status: "pending" as const }))
    ...
    try {
      for (let i = 0; i < queue.length; i++) {
        ...
        const setRowProgress = (p: number) => {
          if (listIdx >= 0) {
            setFiles((prev) => prev.map((row, idx) => (idx === listIdx ? { ...row, progress: p, status: "uploading" } : row)))
          }
        }

        if (listIdx >= 0) {
          setFiles((prev) => prev.map((row, idx) => (idx === listIdx ? { ...row, status: "uploading", progress: 0 } : row)))
        }
        ...
        if (listIdx >= 0) {
          setFiles((prev) => prev.map((row, idx) => (idx === listIdx ? { ...row, progress: 100, status: "done" } : row)))
        }
      }
  ...
  return (
    <div>
      ...
      <input ref={videoRef} type="file" className="d-none" accept="video/mp4,.mp4" onChange={e => { const f = e.target.files?.[0]; if (f) setVideo(f); e.target.value = "" }} />

      {/* Drop zone */}
      <div
        className="border border-dashed rounded p-4 text-center mb-3"
        style={{ cursor: "pointer", borderStyle: "dashed", borderColor: dragOver ? "var(--bs-primary)" : undefined, background: dragOver ? "rgba(13,110,253,0.05)" : undefined }}
      ...
      <div className="mb-3">
        {video ? (
          <div className="d-flex align-items-center gap-2 p-2 rounded" style={{ background: "rgba(13,110,253,0.06)", border: "1px solid rgba(13,110,253,0.15)" }}>
            <i className="bx bx-video text-primary" style={{ fontSize: "1.2rem" }} />
            <div className="flex-grow-1">
              <div style={{ fontSize: "0.82rem", fontWeight: 500 }}>{video.name}</div>
              <div className="text-muted" style={{ fontSize: "0.72rem" }}>{formatSize(video.size)} · Видео-пояснение</div>
            </div>
            <button className="btn btn-sm btn-text-secondary" onClick={() => setVideo(null)}><i className="bx bx-x" /></button>
          </div>
        ) : (
          <button className="btn btn-sm btn-outline-secondary d-flex align-items-center gap-1" onClick={() => videoRef.current?.click()}>
