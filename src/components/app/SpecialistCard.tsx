...
import { SectionLabel, StatusBadge, InfoRow } from "./AppCard"
        ...
        <div className="border-top" style={{ background: "#f8f8f8" }}>
          <div className="px-4 pt-3 pb-4">
            ...
            {hasForm ? (
              <div className="card mb-4">
                <div className="card-body">
                  ...
                  <div className="row g-3">
                    ...
                    {fd?.experience && (
                      <div className="col-sm-6">
                        <InfoRow icon="bx-time-five" label="Опыт работы" value={`${fd.experience} лет`} />
                      </div>
                    )}
                    {fd?.portfolio && (
                      <div className="col-sm-6">
                        <InfoRow icon="bx-link-external" label="Портфолио" value={fd.portfolio} href={fd.portfolio} />
                      </div>
                    )}
                    {fd?.software && (
                      <div className="col-12">
                        <div className="text-uppercase text-muted fw-semibold mb-2" style={{ fontSize: "0.68rem", letterSpacing: "0.07em" }}>
                          Программы
                        </div>
                        <div className="d-flex flex-wrap gap-1">
                          {fd.software.split(",").map(s => s.trim()).filter(Boolean).map(sw => (
                            <span key={sw} className="badge bg-label-primary rounded-pill">{sw}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    {fd?.about && (
                      <div className="col-12">
                        <div className="text-uppercase text-muted fw-semibold mb-1" style={{ fontSize: "0.68rem", letterSpacing: "0.07em" }}>О себе</div>
                        <p className="mb-0" style={{ fontSize: "0.875rem", whiteSpace: "pre-wrap", color: "#555" }}>{fd.about}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="alert alert-warning d-flex align-items-center gap-2 mb-4" style={{ fontSize: "0.875rem" }}>
