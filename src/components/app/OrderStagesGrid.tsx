...
import { STAGE_LABEL, STAGE_ORDER, STAGE_STATUS } from "@/app/orders/[id]/types"
      ...
      {showHeading ? (
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-xs text-muted-foreground">Открывайте этапы по очереди — следующий доступен после принятия предыдущего.</div>
          </div>
                  ...
                  >
                    ...
                    <div className="flex min-h-0 flex-1 flex-col gap-3">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        <Badge variant="outline" className={cn(miniBadgeClass, "text-muted-foreground")}>
                          Закрыто
                        </Badge>
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {isDone ? (
                        ...
                        <CircleDot className="size-6 shrink-0 text-primary" strokeWidth={2} aria-hidden />
                      ) : (
                        <ChevronRight className="size-5 shrink-0 text-muted-foreground/40 opacity-0 transition-opacity group-hover:opacity-100" aria-hidden />
                      )}
                    </div>
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        {insightBadges.length > 0 ? (
                          <div className="flex flex-wrap gap-1" aria-label="Детали этапа">
                            {insightBadges.map((b) => (
                              <Badge key={b.key} variant={b.variant} className={miniBadgeClass}>
                                {b.label}
                              </Badge>
                            ))}
                          </div>
                    ...
                    {showActivityFooter ? (
                      <div className="mt-auto flex flex-col gap-2 border-t border-border/50 pt-4 sm:flex-row sm:items-end sm:justify-between">
                        ...
                        <div className="flex shrink-0 items-center justify-end gap-1.5 sm:flex-col sm:items-end sm:justify-center">
                          {activity.kind !== "none" ? (
                          ...
                          <ChevronRight className="size-4 text-muted-foreground opacity-60 transition-transform group-hover:translate-x-0.5" aria-hidden />
                        </div>
                      </div>
                    ) : (
