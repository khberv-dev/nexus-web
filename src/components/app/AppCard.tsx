...
export function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-uppercase text-muted fw-semibold mb-3" style={{ fontSize: "0.72rem", letterSpacing: "0.08em" }}>
      {children}
    </p>
  )
}
...
export function InfoRow({ icon, label, value, href }: { icon: string; label: string; value: string; href?: string }) {
  return (
    <div className="d-flex align-items-start gap-2 mb-2">
      ...
      <div>
        <div className="text-uppercase text-muted" style={{ fontSize: "0.7rem", letterSpacing: "0.05em" }}>{label}</div>
        {href ? (
          <a href={href} target="_blank" rel="noopener noreferrer" className="text-primary" style={{ fontSize: "0.875rem" }}>{value}</a>
        ) : (
