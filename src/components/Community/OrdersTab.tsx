...
import type { OrderWithRelations, UrgentItem, ActItem } from "./types"
import { STAGE_LABELS, ORDER_STATUS_MAP, ORDER_HUE, DISCOVER_HUES } from "./types"
...
const STAGE_SHORT: Record<StageType, string> = { CONCEPT: "К", PLANNING: "П", VISUALIZATION: "В", DOCUMENTATION: "РД", SPECIFICATION: "С" }
      ...
      {STAGE_ORDER.map((type, i) => {
        ...
        return (
          <React.Fragment key={type}>
            {i > 0 && <div className={`dash-card__stage-line dash-card__stage-line--${state === "none" ? "none" : stageMap[STAGE_ORDER[i - 1]] === "APPROVED" ? "done" : "none"}`} />}
            <div className={`dash-card__stage-dot dash-card__stage-dot--${state}`} title={`${STAGE_LABELS[type]}: ${stageMap[type] ?? "не начат"}`}>
...
function ActsBlock({ items, onSign }: { items: ActItem[]; onSign: (stageId: string) => void }) {
  ...
  return (
    <DashSignalListBlock className="dash-acts" iconClass="bx-file-blank" title="Акты к подписанию" count={items.length}>
      <ul className="dash-acts__list">
        {items.map(({ order, stage }) => (
          <li key={`${order.id}-${stage.type}`} className="dash-acts__item">
            ...
            <button className="dash-acts__action" onClick={() => onSign(stage.id)} style={{ background: "none", border: "none", cursor: "pointer", fontFamily: "inherit" }}>
...
function UrgentBlock({ items }: { items: UrgentItem[] }) {
  ...
  return (
    <DashSignalListBlock className="dash-urgent" iconClass="bx-error-circle" title="Требует внимания" count={items.length}>
        ...
        {items.map(({ order, stage }) => (
          <li key={`${order.id}-${stage.type}`} className="dash-urgent__item">
            ...
            <div className="dash-urgent__info">
              ...
              <p className="dash-urgent__sub">Заказ #{order.id.slice(-6).toUpperCase()} · {LABEL[stage.status] ?? stage.status}</p>
            </div>
            <a href={`/work/${order.id}`} className="dash-urgent__action">Загрузить <i className="bx bx-upload" /></a>
          </li>
        ))}
      </ul>
    </DashSignalListBlock>
          ...
          {orders.slice(0, 6).map(order => {
            ...
            return (
              <li key={order.id} className="dash-list__item">
                <div className="dash-list__thumb" style={{ background: `linear-gradient(135deg, hsl(${hue},60%,58%), hsl(${hue + 35},60%,48%))` }}>
...
export function OrdersCol2({ orders, urgentItems, actItems, onSignAct }: {
  ...
  return (
    <>
      ...
      <DashStatsRow
        items={[
          { label: "Всего проектов", value: orders.length, icon: "bx-folder", bg: "var(--dash-accent-bg)", color: "var(--dash-accent)" },
          { label: "Завершено", value: orders.filter(o => o.status === "DONE").length, icon: "bx-check-circle", bg: "var(--dash-success-bg)", color: "var(--dash-success)" },
          { label: "В работе", value: orders.filter(o => o.status === "ACTIVE").length, icon: "bx-time-five", bg: "var(--dash-warn-bg)", color: "var(--dash-warn)" },
        ]}
      />
      ...
      <DashSectionCard title="Активные заказы">
        <ul className="dash-cards">
          {activeOrders.length === 0 ? (
            <li className="dash-card dash-card--placeholder"><div className="dash-card__body"><h3 className="dash-card__heading">Нет проектов</h3></div></li>
