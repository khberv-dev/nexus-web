...
import { PortfolioCardEditorModal, type CardFile, type PortfolioCard } from "./PortfolioCardEditorModal"
            ...
            <div className="card-body d-flex flex-column gap-3" style={{ padding: 12 }}>
              ...
              <HintPanel title="Как пользоваться">
                <ol className="mb-0 ps-3" style={{ marginTop: 6 }}>
                  <li className="mb-1">Создайте проект слева, затем нажмите его плитку в сетке справа.</li>
                  <li className="mb-1">
                    <strong>К работе</strong> — плитки «Новая работа» / существующие работы: в окне работы — название, главное фото, при желании файлы
                    только к этой работе (к кадру, галерее или отдельной сетке по работе).
                  </li>
                  <li className="mb-0">
                    <strong>К проекту целиком</strong> — блок «Материалы проекта» под плитками на большой панели: общие PDF, спецификации и т.д. на всю
          ...
          <div className="card" style={cardShell}>
            <div className="card-body d-flex flex-column gap-3" style={{ padding: 12 }}>
              <div>
                ...
                <span className="fw-semibold text-truncate d-block" style={{ fontSize: 14, color: "var(--dash-text, #f4f4f4)" }}>
                  {selectedProject.name}
                </span>
              </div>
              <ActionButton type="button" icon="bx-chevrons-left" className="btn-sm btn-outline-secondary" onClick={goToProjectsRoot}>
                Все проекты
              </ActionButton>
              ...
              <HintPanel title="Работы и материалы">
                <ul className="mb-0 ps-3" style={{ marginTop: 6 }}>
                  <li className="mb-2">
                    <strong>Плитки</strong> — это работы. Просмотр по клику; «Изменить» — вложения и фото <em>только у этой работы</em>.
                  </li>
                  <li className="mb-0">
                    <strong>Ниже плиток</strong> (на широкой колонке) — «Материалы проекта»: файлы <em>на всю папку</em>, не привязанные к одной работе.
                  </li>
                </ul>
              </HintPanel>
            </div>
      ...
      <div className="dash-col2" id="portfolio-uploader">
        <div className="card" style={cardShell}>
          <div className="card-body d-flex flex-column gap-3" style={{ padding: 14 }}>
            <nav aria-label="Навигация по портфолио" className="d-flex flex-wrap align-items-center justify-content-between gap-2">
              <div className="d-flex flex-wrap align-items-center gap-1 small">
                ...
                {selectedProject && (
                  <>
                    ...
                    <button
                      ...
                      style={{ maxWidth: "min(280px, 46vw)", color: !editingCard ? "var(--dash-text, #f4f4f4)" : undefined }}
                ...
                {editingCard && (
                  <>
                    <span className="text-muted user-select-none">/</span>
                    <span className="small text-truncate fw-semibold" style={{ maxWidth: "min(200px, 40vw)" }} title={editingCard.title}>
                      {editingCard.title}
                    </span>
                  </>
                )}
              ...
              {selectedProject && (
                <ActionButton type="button" className="btn-sm btn-outline-secondary d-none d-md-inline-flex" onClick={goToProjectsRoot}>
                  Все проекты
                </ActionButton>
              )}
            </nav>
            ...
            {!selectedProject ? (
              <>
                {loadingProjects ? (
                  ...
                  <div
                    ...
                    style={{ border: "1px dashed rgba(255,255,255,0.15)", background: "rgba(12,16,30,0.25)" }}
                  >
                    <i className="bx bx-folder-plus d-block mb-2" style={{ fontSize: 28, opacity: 0.65 }} aria-hidden />
                    <p className="small text-muted mb-0" style={{ lineHeight: 1.55 }}>
                      Проектов нет. Создайте папку слева кнопкой <strong>«Добавить проект»</strong>, затем откройте её здесь.
                    </p>
                  </div>
                ) : (
                  <>
                    <p className="small text-muted mb-2">
                      Плитки в том же формате, что и работы. Нажмите плитку проекта — откроется сетка работ. Число справа внизу — сколько работ в
                      папке.
                    </p>
                        ...
                        return (
                          ...
                          >
                            <div className="pf-port-grid__cover">
                              ...
                              >
                                <i className="bx bx-folder" style={{ fontSize: 52, opacity: 0.38, color: "#b4bce8" }} aria-hidden />
                              </div>
                            </div>
              ...
              <>
                <p className="small text-muted mb-2" style={{ lineHeight: 1.45 }}>
                  <strong>Сверху</strong> — работы: первая плитка «Новая работа», остальные — просмотр; файлы к конкретной работе — в её окне.{" "}
                  <strong>Снизу</strong> — «Материалы проекта»: общие вложения на всю папку (отдельно от работ).
                </p>
                            ...
                            <div className="pf-port-grid__cover">
                              {cover ? (
                                ...
                                <div className="d-flex align-items-center justify-content-center h-100 w-100 text-muted">
                                  <i className="bx bx-image" style={{ fontSize: 32, opacity: 0.35 }} aria-hidden />
                                </div>
                              )}
                            </div>
                            <div className="pf-port-grid__shade" aria-hidden />
            ...
            <PortfolioCardEditorModal
              ...
              onSuccess={() => {
                ...
                if (editorMode === "create" && selectedProject) {
                  setProjects((prev) =>
                    prev.map((p) =>
                      p.id === selectedProject.id ? { ...p, _count: { cards: p._count.cards + 1 } } : p,
                    ),
                  )
                }
              }}
