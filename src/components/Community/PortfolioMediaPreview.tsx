...
export function PortfolioRemoteFilePreview({
  ...
  if (loading && !url) {
    return (
      <div style={{ ...boxStyle }}>
        <i className="bx bx-loader-alt bx-spin" style={{ fontSize: 22, color: "rgba(255,255,255,0.5)" }} aria-hidden />
      </div>
    )
  }
    ...
    return (
      <div style={{ ...boxStyle, position: "relative", padding: 0 }}>
        ...
        <span
          ...
          style={{ background: "linear-gradient(transparent, rgba(0,0,0,0.75))", color: "#fff", fontSize: 10, padding: "4px 2px" }}
        >
          видео
        </span>
      </div>
  ...
  const open =
    url && !err ? (
      <a href={url} target="_blank" rel="noopener noreferrer" className="small" style={{ color: "var(--bs-primary, #8b7cf6)" }}>
        Открыть
      </a>
    ) : (
  ...
  return (
    <div style={{ ...boxStyle, flexDirection: "column", gap: 4 }} className="d-flex">
      <i
        ...
        style={{ fontSize: Math.min(32, (fillParent ? 120 : size) * 0.28), opacity: 0.75, color: "rgba(255,255,255,0.75)" }}
        aria-hidden
      />
      {open}
    </div>
...
export function PortfolioLocalFilePreview({ file, size = 112 }: { file: File; size?: number }) {
  ...
  return (
    <div style={{ ...boxStyle, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <i className="bx bx-file" style={{ fontSize: 28, opacity: 0.7, color: "rgba(255,255,255,0.75)" }} aria-hidden />
    </div>
  )
}
