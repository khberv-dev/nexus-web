...
import { AppModal, ActionButton } from "@/components/app/AppCard"
...
function DashSectionLabel({ children }: { children: ReactNode }) {
  return (
    <p className="fw-semibold text-uppercase mb-2" style={{ fontSize: "0.7rem", letterSpacing: "0.08em", color: "rgba(255,255,255,0.45)" }}>
      {children}
    </p>
  )
}
...
export function PortfolioCardEditorModal({
  ...
  return (
    <AppModal open={open} onClose={handleClose} maxWidth={640} variant="dark">
      <div className="portfolio-dash-modal" style={{ display: "flex", flexDirection: "column", maxHeight: "min(88vh, 720px)" }}>
        <div
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "16px 20px" }}>
          {error && (
            <div
              ...
              role="alert"
              style={{ background: "rgba(220,53,69,0.12)", border: "1px solid rgba(220,53,69,0.35)", color: "#ffc9c9" }}
            >
              {error}
            </div>
          )}
          ...
          <div className="d-flex flex-wrap align-items-start gap-3 mb-3">
            ...
            <div className="d-flex flex-column gap-2 flex-grow-1" style={{ minWidth: 200 }}>
              ...
              />
              <ActionButton type="button" icon="bx-image-add" onClick={() => mainInputRef.current?.click()} disabled={saving}>
                Выбрать файл
              </ActionButton>
              ...
              {mainPick && (
                <ActionButton type="button" variant="danger" onClick={removePendingMain} disabled={saving}>
                  Сбросить выбор
                </ActionButton>
              )}
              {mode === "edit" && localMain && !mainPick && (
                <ActionButton type="button" variant="danger" icon="bx-trash" onClick={() => void removeMainOnServer()} disabled={saving}>
                  Убрать основной
                </ActionButton>
              )}
            </div>
          ...
          <p className="small mb-2" style={{ marginTop: -6, color: "rgba(255,255,255,0.5)" }}>
            PDF, MP4, ZIP, RAR, DWG, DXF и доп. изображения. Два варианта: привязать файл к главному фото или к ранее добавленному кадру (изображение),
          ...
          <div className="d-flex flex-wrap align-items-center gap-2 mb-2">
            <input
              ...
              onChange={(e) => {
                ...
                if (list.length) {
                  setExtraDrafts((prev) => [...prev, ...list.map((file) => ({ file, link: "standalone" as const }))])
                }
                e.target.value = ""
              }}
            />
            <ActionButton type="button" icon="bx-paperclip" onClick={() => extraInputRef.current?.click()} disabled={saving}>
              Добавить файлы
            </ActionButton>
          </div>
              ...
              {extraDrafts.map((row, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="d-flex flex-column gap-1 flex-grow-1" style={{ minWidth: 120 }}>
                      ...
                      {isVisualLocalFile(row.file) && (
                        <span className="small" style={{ color: "rgba(255,255,255,0.45)" }}>
                          Кадр галереи (к нему можно привязать материалы ниже по списку)
                        </span>
                      )}
                    </div>
                    <button type="button" className="btn btn-link btn-sm p-0 text-danger text-nowrap" onClick={() => removeExtraDraft(i)} disabled={saving}>
                      убрать
                    </button>
                  </div>
                )
          ...
          {mode === "edit" && localAttachments.length > 0 && (
            <>
              ...
              <p className="small mb-2" style={{ color: "rgba(255,255,255,0.45)" }}>
                Для файлов, которые не являются кадром галереи, выберите привязку. Нажмите «Открыть» на превью, чтобы открыть в новой вкладке.
              </p>
                  ...
                  return (
                    ...
                    >
                      <PortfolioRemoteFilePreview
                        ...
                        rounded={8}
                      />
                      <span className="small text-truncate flex-grow-1" style={{ minWidth: 100, color: "rgba(255,255,255,0.8)" }}>
                      ...
                      {isGalleryImage && (
                        <span className="small text-nowrap" style={{ color: "rgba(255,255,255,0.45)" }}>
                          Кадр галереи
                        </span>
                      )}
                      <button
        ...
        >
          <button type="button" className="btn btn-sm btn-outline-light" onClick={handleClose} disabled={saving}>
            Отмена
          </button>
          <button type="button" className="btn btn-sm btn-primary d-inline-flex align-items-center gap-1" onClick={() => void submit()} disabled={saving}>
