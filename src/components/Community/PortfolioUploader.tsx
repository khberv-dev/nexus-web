...
import { AppModal, ActionButton, SectionLabel } from "@/components/app/AppCard"
...
function AiChatDrawer({
  ...
  return createPortal(
    <div style={{ position: "fixed", inset: 0, overflow: "hidden", pointerEvents: open ? "auto" : "none", zIndex: 50 }}>
      {/* Backdrop */}
      <div
        onClick={onClose}
        style={{
      ...
      <div style={{
        ...
        {/* Шапка */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1.1rem 1.4rem", borderBottom: "1px solid rgba(255,255,255,0.07)", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.5em" }}>
            <span style={{ fontSize: "1rem" }}>✨</span>
            <span style={{ color: "#f4f4f4", fontSize: "0.92rem", fontWeight: 500 }}>AI-помощник</span>
            <span style={{ background: "rgba(121,40,202,0.25)", borderRadius: 100, color: "rgba(255,255,255,0.4)", fontSize: "0.65rem", fontWeight: 600, letterSpacing: "0.05em", padding: "0.2em 0.6em", textTransform: "uppercase" }}>
              описание
            </span>
          </div>
          <button onClick={onClose} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 6, color: "rgba(255,255,255,0.45)", cursor: "pointer", fontSize: "0.9rem", padding: "0.3em 0.55em" }}>✕</button>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "1rem 1.4rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          {messages.map((m, i) => {
            ...
            return (
              <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: isUser ? "flex-end" : "flex-start", gap: "0.3rem" }}>
                <div style={{
                ...
                {ready && (
                  <div style={{ maxWidth: "88%", width: "100%" }}>
                    <div style={{ background: "rgba(52,211,153,0.06)", border: "1px solid rgba(52,211,153,0.2)", borderRadius: 8, padding: "0.65rem 0.9rem", marginBottom: "0.4rem" }}>
                      <div style={{ color: "rgba(52,211,153,0.7)", fontSize: "0.65rem", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: "0.35rem" }}>Готовое описание</div>
                      <p style={{ color: "#e8e8f0", fontSize: "0.83rem", lineHeight: 1.5, margin: 0, whiteSpace: "pre-wrap" }}>{ready}</p>
                      <div style={{ color: "rgba(255,255,255,0.25)", fontSize: "0.7rem", marginTop: "0.35rem" }}>{ready.length} / 500 символов</div>
                    </div>
                    <button
                      onClick={() => { onApply(ready.slice(0, DESC_MAX)); onClose() }}
                      style={{ background: "rgba(52,211,153,0.15)", border: "1px solid rgba(52,211,153,0.3)", borderRadius: 6, color: "rgba(52,211,153,0.9)", cursor: "pointer", fontSize: "0.8rem", fontFamily: "inherit", fontWeight: 500, padding: "0.4em 0.9em" }}
                    >
                      Применить →
                    </button>
                  </div>
          ...
          {loading && (
            <div style={{ display: "flex", gap: "0.3rem", padding: "0.5rem 0" }}>
              {[0, 1, 2].map(i => (
                <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: "rgba(255,255,255,0.3)", animation: `bounce 1.2s ${i * 0.2}s ease-in-out infinite` }} />
              ))}
            </div>
          )}

          {error && (
            <div style={{ background: "rgba(240,20,20,0.07)", border: "1px solid rgba(240,20,20,0.2)", borderRadius: 8, color: "rgba(255,100,100,0.8)", fontSize: "0.83rem", padding: "0.7rem 0.9rem" }}>
              {error}
            </div>
          )}
...
const TABS: { id: Tab; label: string; icon: string; accept: string; hint: string }[] = [
  { id: "PORTFOLIO", label: "Фото и рендеры", icon: "bx-image-alt", accept: ".jpg,.jpeg,.png", hint: "JPG, PNG · до 500 МБ" },
  { id: "DOCUMENT", label: "Материалы", icon: "bx-file", accept: ".pdf,.dwg,.dxf,.zip", hint: "PDF, DWG, DXF, ZIP · до 500 МБ" },
]
...
function PreviewModal({ file, url, onClose, onSave }: {
  ...
  return (
    <AppModal open onClose={onClose} maxWidth={960}>
      {isImage(file) && (
        <div style={{ background: "#111", maxHeight: "55vh", overflow: "hidden", borderRadius: "16px 16px 0 0", display: "flex", alignItems: "center", justifyContent: "center" }}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={url} alt={file.title ?? file.filename} style={{ maxWidth: "100%", maxHeight: "55vh", objectFit: "contain" }} />
        </div>
      )}
      <div style={{ padding: "20px 24px 24px" }}>
        <div className="d-flex align-items-start justify-content-between gap-3 mb-3">
          <div style={{ flex: 1, minWidth: 0 }}>
            {editing
              ? <input className="form-control fw-semibold mb-1" style={{ fontSize: 17 }} value={title} onChange={e => setTitle(e.target.value)} placeholder="Название работы" autoFocus />
              : <h5 className="mb-0 fw-semibold" style={{ fontSize: 17 }}>{file.title || file.filename}</h5>}
            <small className="text-muted">{fmt(file.size)}{file.size ? " · " : ""}{new Date(file.createdAt).toLocaleDateString("ru-RU")}</small>
          </div>
          <div className="d-flex gap-2 flex-shrink-0 align-items-center">
            {!editing && <ActionButton icon="bx-edit" onClick={() => setEditing(true)}>Изменить</ActionButton>}
            <ActionButton icon="bx-link-external" onClick={() => window.open(url, "_blank")}>Открыть</ActionButton>
            <button className="btn btn-sm btn-outline-secondary px-2" onClick={onClose}><i className="bx bx-x" style={{ fontSize: 18 }} /></button>
          </div>
        </div>
        <div>
          <SectionLabel>Описание</SectionLabel>
          {editing
            ? <textarea className="form-control" rows={3} value={description} onChange={e => setDescription(e.target.value)} placeholder="Объект, стиль, площадь, использованные решения…" />
        ...
        {editing && (
          <div className="d-flex gap-2 mt-3">
            <ActionButton variant="primary" icon="bx-check" onClick={handleSave} disabled={saving}>{saving ? "Сохранение…" : "Сохранить"}</ActionButton>
            <ActionButton onClick={() => { setEditing(false); setTitle(file.title ?? ""); setDescription(file.description ?? "") }}>Отмена</ActionButton>
          </div>
        )}
      </div>
    </AppModal>
...
export default function PortfolioUploader() {
  ...
  const uploadPending = async () => {
    ...
    try {
      ...
      try { resJson = JSON.parse(resText) } catch { throw new Error(`Ошибка сервера: ${resText.slice(0, 120)}`) }
  ...
  const deleteFile = async (id: string) => {
    ...
    setPreviewUrls(prev => { const n = { ...prev }; delete n[id]; return n })
    if (modalFile?.id === id) {
      setModalFile(null)
      setModalUrl("")
    }
    ...
    return (
      <div key={f.id} className={isList ? "col-12" : "col-sm-6 col-xl-4"}>
        ...
        >
          {/* Превью/иконка */}
          {(!isList && img) ? (
            <div style={{ height: 180, overflow: "hidden", background: "rgba(91,79,207,0.06)", position: "relative" }}>
              {preview
                // eslint-disable-next-line @next/next/no-img-element
                ? <img src={preview} alt={f.title ?? f.filename} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                : <div className="d-flex align-items-center justify-content-center h-100"><i className="bx bx-image text-muted" style={{ fontSize: 48 }} /></div>}
              <div className="portfolio-overlay"><i className="bx bx-zoom-in" style={{ fontSize: 28, color: "#fff" }} /></div>
            </div>
          ) : (
            <div style={{
              ...
              {img && preview
                // eslint-disable-next-line @next/next/no-img-element
                ? <img src={preview} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: isList ? "8px 0 0 8px" : 8 }} />
                : <i className={`bx ${img ? "bx-image" : "bx-file"} text-primary`} style={{ fontSize: 20 }} />}
            </div>
          )}
          ...
          <div style={{ padding: isList ? "10px 12px" : "10px 14px 12px", flex: 1, minWidth: 0 }}>
            <div className="d-flex align-items-start justify-content-between gap-2">
              <div style={{ minWidth: 0 }}>
                ...
                {f.description && (
                  <p className="text-muted mb-0" style={{ fontSize: "0.78rem", display: "-webkit-box", WebkitLineClamp: isList ? 1 : 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                    {f.description}
                  </p>
                )}
              ...
              <div className="d-flex gap-1 flex-shrink-0">
                <button className="up-icon-btn" onClick={e => { e.stopPropagation(); setConfirmDeleteId(f.id) }} aria-label="Удалить">
        ...
        >
          ...
          <div className="pf-carousel__overlay">
            ...
            <div className="pf-carousel__actions">
              <button className="up-icon-btn" onClick={e => { e.stopPropagation(); void setConfirmDeleteId(f.id) }} aria-label="Удалить">
                <i className="bx bx-trash text-danger" />
              </button>
            </div>
          </div>
    ...
    <div>
      ...
      <div className="d-flex align-items-center justify-content-between mb-3">
        <div className="d-flex gap-2">
          {TABS.map(t => (
            <button key={t.id} onClick={() => { setTab(t.id); clearPending() }}
      ...
      <div className="pf-upload-block">
        ...
        <div className="card mb-0" style={{ border: "1px solid rgba(255,255,255,0.08)", boxShadow: "none", background: "rgba(20,25,40,0.28)" }}>
          <div className="card-body p-3" style={{ background: "transparent" }}>
            <div className="row g-3 align-items-stretch">
              <div className="col-md-5">
                {pendingFile ? (
                  <div style={{ position: "relative", borderRadius: 10, overflow: "hidden", height: "100%", minHeight: 140, background: "rgba(91,79,207,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
                    {pendingPreview ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={pendingPreview} alt="preview" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                    ) : (
                      <div className="d-flex flex-column align-items-center justify-content-center h-100 gap-2 p-3">
                        <i className="bx bx-file text-primary" style={{ fontSize: 36 }} />
                        <p className="mb-0 small fw-medium text-truncate text-center" style={{ maxWidth: "90%" }}>{pendingFile.name}</p>
                        <small className="text-muted">{fmt(pendingFile.size)}</small>
                      </div>
                    )}
                    {!uploading && (
                      <button onClick={clearPending} style={{ position: "absolute", top: 6, right: 6, width: 26, height: 26, borderRadius: "50%", border: "none", background: "rgba(0,0,0,0.55)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 14 }} aria-label="Отменить выбор">
                        <i className="bx bx-x" />
                      </button>
                    )}
                    {uploading && (
                      <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8 }}>
                        <i className="bx bx-loader-alt bx-spin" style={{ fontSize: 28, color: "#fff" }} />
                        <div style={{ width: "70%" }}>
                          <div className="progress" style={{ height: 4, borderRadius: 10, background: "rgba(255,255,255,0.2)" }}>
                            <div className="progress-bar" style={{ width: `${progress}%`, background: "#fff", transition: "width 0.2s" }} />
                          </div>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 10, height: "100%", minHeight: 140,
                  ...
                  >
                    <i className={`bx bx-cloud-upload ${dragging ? "text-primary" : "text-muted"}`} style={{ fontSize: 30 }} />
                    <p className="mb-0 fw-medium small" style={{ color: dragging ? "var(--dash-accent, var(--bs-primary))" : "var(--dash-text, #201d1d)" }}>
                      {dragging ? "Отпустите для выбора" : "Перетащите файл или нажмите"}
                    </p>
                    <small className="text-muted">{currentTab.hint}</small>
                  </div>
                )}
                <input ref={inputRef} type="file" accept={currentTab.accept} className="d-none" onChange={e => e.target.files?.[0] && selectFile(e.target.files[0])} />
              </div>

              <div className="col-md-7 d-flex flex-column gap-2">
                <div>
                  <label className="form-label small fw-medium mb-1">Название работы</label>
                  <input className="form-control form-control-sm" placeholder="Офис на ул. Ленина, 80 м²" value={title} onChange={e => setTitle(e.target.value)} disabled={uploading || !pendingFile} />
                </div>
                <div className="flex-grow-1 d-flex flex-column">
                  <div className="d-flex align-items-center justify-content-between mb-1">
                    <label className="form-label small fw-medium mb-0">Описание <span className="text-muted fw-normal" style={{ fontSize: "0.73rem" }}>(необязательно)</span></label>
                    <small className="text-muted" style={{ fontSize: "0.7rem" }}>{description.length} / {DESC_MAX}</small>
                  </div>
                  <textarea className="form-control form-control-sm flex-grow-1" rows={3} maxLength={DESC_MAX} placeholder={pendingFile ? "Стиль, площадь, особенности проекта…" : "Сначала выберите файл"} value={description} onChange={e => setDescription(e.target.value.slice(0, DESC_MAX))} disabled={uploading || !pendingFile} style={{ resize: "none" }} />
                </div>
                <div className="d-flex align-items-center gap-2 mt-1 flex-wrap">
                  <button className="btn btn-primary btn-sm d-flex align-items-center gap-1" onClick={uploadPending} disabled={!pendingFile || uploading} style={{ minWidth: 120 }}>
                    {uploading ? <><i className="bx bx-loader-alt bx-spin" />Загрузка…</> : <><i className="bx bx-upload" />Загрузить</>}
                  </button>
                  <button type="button" className="btn btn-sm d-flex align-items-center gap-1" style={{ fontSize: "0.8rem", padding: "0.3rem 0.65rem", background: "rgba(91,79,207,0.06)", border: "1px solid rgba(91,79,207,0.25)", borderRadius: 6, color: "#5b4fcf", whiteSpace: "nowrap" }} onClick={() => setDrawerOpen(true)} disabled={uploading || !pendingFile} title="Составить описание с помощью AI">
        ...
        <div className="pf-gallery-block__inner">
          <div className="d-flex align-items-center justify-content-between gap-2 mb-3">
            ...
            <div className="d-flex align-items-center gap-2">
              {files.length > 0 && <span className="badge bg-label-secondary" style={{ fontSize: "0.7rem" }}>{files.length}</span>}
              {tab === "PORTFOLIO" && files.length > 0 && (
                <div className="btn-group btn-group-sm">
                  <button className={`btn ${view === "grid" ? "btn-primary" : "btn-outline-secondary"}`} onClick={() => setView("grid")} title="Блок">
                    <i className="bx bx-grid-alt" />
                  </button>
                  <button className={`btn ${view === "list" ? "btn-primary" : "btn-outline-secondary"}`} onClick={() => setView("list")} title="Списком">
      ...
      {/* ── Модальное превью ── */}
      {modalFile && modalUrl && (
        <PreviewModal file={modalFile} url={modalUrl} onClose={() => { setModalFile(null); setModalUrl("") }} onSave={saveFileMeta} />
      )}
