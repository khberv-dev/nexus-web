...
import type { PortfolioCard, CardFile, CardAttachment } from "./PortfolioCardEditorModal"
    ...
    return (
      ...
      >
        ...
        {u && isImage(a.file) && (
          ...
          <img src={u} alt="" style={{ maxWidth: "100%", maxHeight: 320, borderRadius: 8, objectFit: "contain" }} />
        )}
        {u && isVideo(a.file) && (
          <video src={u} controls playsInline style={{ maxWidth: "100%", maxHeight: 320, borderRadius: 8 }} />
        )}
        {u && isPdf(a.file) && (
          <a href={u} target="_blank" rel="noopener noreferrer" className="btn btn-sm btn-outline-light" download>
      ...
      >
        ...
        >
          <div className="min-w-0">
            ...
            {c.description && (
              <p className="small mb-0" style={{ color: "rgba(255,255,255,0.55)", whiteSpace: "pre-wrap", maxHeight: 72, overflow: "auto" }}>
                {c.description}
              </p>
            )}
          </div>
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "16px 18px" }}>
          <p className="small text-uppercase fw-semibold mb-2" style={{ letterSpacing: "0.06em", color: "rgba(255,255,255,0.45)" }}>
            Основной материал
          </p>
          <div className="mb-4 d-flex justify-content-center" style={{ minHeight: 200 }}>
            {c.mainFile && mainUrl && isImage(c.mainFile) && (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={mainUrl} alt="" style={{ maxWidth: "100%", maxHeight: "55vh", borderRadius: 12, objectFit: "contain" }} />
            )}
            {c.mainFile && mainUrl && isVideo(c.mainFile) && (
              <video src={mainUrl} controls playsInline style={{ maxWidth: "100%", maxHeight: "55vh", borderRadius: 12 }} />
            )}
            {c.mainFile && mainUrl && !isImage(c.mainFile) && !isVideo(c.mainFile) && (
              <a href={mainUrl} target="_blank" rel="noopener noreferrer" className="btn btn-outline-light">
                <i className="bx bx-link-external me-1" />
          ...
          {underMain.length > 0 && (
            <div className="mb-4">
              ...
              <div className="d-flex flex-column gap-2">{underMain.map((a) => renderAttachmentBlock(a))}</div>
            </div>
          )}
            ...
            <div className="mb-4">
              ...
              <div className="d-flex flex-column gap-4">
                {galleryImages.map((gi) => {
                  ...
                  return (
                    <div key={gi.id} className="rounded-3 p-2" style={{ border: "1px solid rgba(255,255,255,0.1)", background: "rgba(0,0,0,0.15)" }}>
                      <div className="small fw-medium mb-2 text-truncate" style={{ color: "rgba(255,255,255,0.75)" }}>
                        {name}
                      </div>
                      {!u && <span className="small text-muted">Загрузка…</span>}
                      {u && (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={u} alt="" style={{ maxWidth: "100%", maxHeight: 280, borderRadius: 8, objectFit: "contain" }} />
                      )}
                      {nested.length > 0 && (
                        <div className="mt-2 pt-2" style={{ borderTop: "1px solid rgba(255,255,255,0.08)" }}>
                          ...
                          <div className="d-flex flex-column gap-2">{nested.map((a) => renderAttachmentBlock(a))}</div>
                        </div>
                      )}
                    </div>
                  )
