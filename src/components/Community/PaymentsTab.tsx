...
import type { PaymentWithRelations, SpecContract, SpecAct } from "./types"
import { PAYMENT_BADGE, PAYMENT_HUE, STAGE_LABELS, DISCOVER_HUES } from "./types"
...
function ContractActions({ contract: c }: { contract: SpecContract }) {
  ...
  const uploadSigned = async () => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.error ?? "Ошибка загрузки") }
  ...
  return (
    <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" }}>
      {canDownload && (
        <button onClick={download} disabled={loading} style={{ background: "none", border: "1px solid var(--dash-border)", borderRadius: 6, padding: "3px 10px", fontSize: "0.72rem", cursor: "pointer", color: "var(--dash-accent, #5b4fcf)", fontFamily: "inherit" }}>
          <i className="bx bx-download" style={{ marginRight: 4 }} />{loading ? "..." : "Скачать"}
        </button>
      )}
      {canSign && (
        <button onClick={uploadSigned} disabled={loading} style={{ background: "rgba(91,79,207,0.1)", border: "1px solid rgba(91,79,207,0.3)", borderRadius: 6, padding: "3px 10px", fontSize: "0.72rem", cursor: "pointer", color: "#5b4fcf", fontFamily: "inherit", fontWeight: 600 }}>
          <i className="bx bx-upload" style={{ marginRight: 4 }} />{loading ? "Загрузка..." : "Загрузить подписанный"}
        </button>
      )}
    </div>
  )
...
export function PaymentsCol1({ payments, formData, contracts, acts }: {
  ...
  const stats = [
    { label: "Выплачено", value: totalReleased, icon: "bx-check-circle", color: "var(--dash-success)", bg: "var(--dash-success-bg)" },
    { label: "Удержано", value: totalHeld, icon: "bx-time-five", color: "var(--dash-warn)", bg: "var(--dash-warn-bg)" },
    { label: "Ожидает", value: totalPending, icon: "bx-credit-card", color: "var(--dash-accent)", bg: "var(--dash-accent-bg)" },
  ].filter(s => s.value > 0)
  ...
  return (
    <div>
        ...
        <div className="dash-stats-row" style={{ marginBottom: 12 }}>
          {stats.map(s => (
            <div key={s.label} className="dash-stat-card">
              <div className="dash-stat-card__icon" style={{ background: s.bg, color: s.color }}><i className={`bx ${s.icon}`} /></div>
              <div><p className="dash-stat-card__value">{(s.value / 100).toLocaleString("ru-RU")} руб.</p><p className="dash-stat-card__label">{s.label}</p></div>
            </div>
          ))}
        </div>
      )}
      ...
      <Section title="Реквизиты" icon="bx-building">
        {hasReqs ? (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 16px", fontSize: "0.78rem" }}>
            {reqRows.map(r => r.value ? (
              <div key={r.label}><span style={{ color: "var(--dash-muted)" }}>{r.label}: </span>{r.value}</div>
            ) : null)}
          </div>
        ) : (
          <p style={{ fontSize: "0.8rem", color: "var(--dash-muted)", margin: 0 }}>Не заполнены — укажите в Настройках</p>
        )}
      </Section>
          ...
          <div className="dash-finance-list">
            {acts.map(a => (
              <div key={a.id} className="dash-finance-card">
                <div className="dash-finance-card__row">
                  <div className="dash-finance-card__meta">
                    <span className="dash-finance-card__title">{STAGE_LABELS[a.stageType as StageType] ?? a.stageType}</span>
                    <span className="dash-finance-card__sub">#{a.orderId.slice(-6).toUpperCase()}</span>
                  </div>
                  <StatusBadge variant={a.signedAt ? "done" : "current"} label={a.signedAt ? "Подписан" : "Ожидает"} />
                </div>
            ...
            {payments.map(p => {
              ...
              return (
                <div key={p.id} className="dash-finance-card">
                  <div className="dash-finance-card__row">
                    <div className="dash-finance-card__meta">
                      <span className="dash-finance-card__title">#{p.order.id.slice(-6).toUpperCase()}</span>
                      <span className="dash-finance-card__sub">{new Date(p.createdAt).toLocaleDateString("ru-RU")}</span>
                    </div>
                    <span className="dash-finance-card__sum">{(p.amount / 100).toLocaleString("ru-RU")} руб.</span>
                  </div>
    ...
    <>
      <div className="dash-cards-container">
        ...
        <ul className="dash-cards">
          {[
            { label: "Выплачено", value: `${(totalReleased / 100).toLocaleString("ru-RU")} руб.`, hue: 140, icon: "bx-check-circle" },
            { label: "Удержано", value: `${(totalHeld / 100).toLocaleString("ru-RU")} руб.`, hue: 200, icon: "bx-time-five" },
            ...
            <li key={s.label} className="dash-card" style={{ "--hue": s.hue } as React.CSSProperties}>
              ...
              <div className="dash-card__body"><h3 className="dash-card__heading">{s.value}</h3><p className="dash-card__sub">{s.label}</p></div>
            </li>
          ))}
        </ul>
      </div>
          ...
          {statusBreakdown.map((item, i) => (
            <li key={item.label} className="dash-discover__place">
              ...
              <div className="dash-discover__more">
                <div className="dash-discover__more-icon" style={{ background: `linear-gradient(20deg, hsl(${DISCOVER_HUES[i % DISCOVER_HUES.length].h1},72%,52%), hsl(${DISCOVER_HUES[i % DISCOVER_HUES.length].h2},72%,44%))` }}>
