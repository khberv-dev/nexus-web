...
import { OrderData, ORDER_STATUS, BRIEF_PLACEHOLDERS, OrderStatus } from "./types"
...
export default function OrderDetailClient({
  ...
  const requestHelp = async () => { setHelpRequested(true); await doSave({ _briefHelpRequested: true }) }

  const st = ORDER_STATUS[order.status as OrderStatus] ?? { label: order.status, color: "var(--dash-muted)", bg: "var(--dash-border)" }
  ...
  const submitBrief = async () => {
    setSubmitting(true); await doSave()
    const res = await fetch(`/api/orders/${order.id}/brief/submit`, { method: "POST" })
    if (res.ok) { setOrder(prev => ({ ...prev, status: "BRIEFING" })); setSubmitDone(true) }
    setSubmitting(false)
  }
  ...
  const handleUploadSignedAct = async (stageId: string, file: File): Promise<{ success: boolean; error?: string }> => {
    const formData = new FormData()
    formData.append("file", file)
        ...
        <main className="dash-main">
          <div className="dash-main__scroll">
            ...
            <div style={{ marginBottom: 20 }}>
              <h1 style={{ fontSize: "1.3rem", fontWeight: 600, color: "var(--dash-text)", margin: "0 0 4px" }}>
            ...
            {(order.status === "BRIEFING" || order.status === "BRIEF_REVIEW") && (
              <div style={{ background: "var(--dash-accent-bg)", border: "1px solid var(--dash-accent-border)", borderRadius: 10, padding: "12px 16px", marginBottom: 16, display: "flex", gap: 12, alignItems: "flex-start", fontSize: "0.85rem", lineHeight: 1.45 }}>
                <i className="bx bx-mail-send" style={{ color: "var(--dash-accent)", fontSize: "1.15rem", flexShrink: 0, marginTop: 2 }} />
                <span style={{ color: "var(--dash-text2)", overflowWrap: "anywhere" }}>Заявка отправлена — подбираем специалиста</span>
              </div>
            )}
            {clientReviewCount > 0 && (
              <div style={{ background: "var(--dash-warn-bg)", border: "1px solid var(--dash-warn)", borderRadius: 10, padding: "12px 16px", marginBottom: 16, display: "flex", gap: 12, alignItems: "flex-start", fontSize: "0.85rem", lineHeight: 1.45 }}>
                <i className="bx bx-bell" style={{ color: "var(--dash-warn)", fontSize: "1.15rem", flexShrink: 0, marginTop: 2 }} />
            ...
            <div className="dash-content">
              ...
              <div className="dash-col1">
                ...
                {order.stages
                  ...
                  .map((stage) => (
                    <div key={stage.id} id={`order-act-${stage.id}`} style={{ marginBottom: 16, scrollMarginTop: 88 }}>
                ...
                {order.payments.length > 0 && (
                  <div style={{ background: "var(--dash-surface)", borderRadius: 10, padding: "14px 16px", marginBottom: 16, border: "1px solid var(--dash-border)" }}>
                    <div className="dash-list-heading-wrap" style={{ marginBottom: 10 }}>
                      <h2 className="dash-list-heading">Платежи</h2>
                    </div>
                    {order.payments.map(p => (
                      <div key={p.id} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid var(--dash-border)", fontSize: "0.82rem" }}>
                        <span style={{ color: "var(--dash-text2)" }}>{(p.amount / 100).toLocaleString("ru-RU")} руб.</span>
                ...
                <div id="order-brief" style={{ scrollMarginTop: 88 }}>
                  ...
                  <DashBriefCard
                    ...
                    headingRight={isEditable ? (
                      <span style={{ fontSize: "0.72rem", color: briefSaving ? "var(--dash-muted)" : briefSaved ? "var(--dash-success)" : "transparent" }}>
                  ...
                  {isEditable && (
                    <div style={{ marginTop: 16 }}>
                      {submitDone ? (
                        <div style={{ background: "var(--dash-success-bg)", border: "1px solid var(--dash-success)", borderRadius: 10, padding: "10px 14px", display: "flex", gap: 8, alignItems: "center" }}>
                          <i className="bx bx-check-circle" style={{ color: "var(--dash-success)" }} />
                          <span style={{ fontSize: "0.82rem", color: "var(--dash-success)", fontWeight: 500 }}>Заявка отправлена!</span>
                        </div>
                      ) : (
                        <button onClick={submitBrief} disabled={submitting || !canSubmit}
                          style={{ width: "100%", padding: "0.65em", borderRadius: 8, border: "none", background: canSubmit ? "var(--dash-accent)" : "var(--dash-border)", color: canSubmit ? "#fff" : "var(--dash-muted)", fontSize: "0.85rem", fontWeight: 600, cursor: canSubmit && !submitting ? "pointer" : "default", fontFamily: "inherit", opacity: submitting ? 0.7 : 1 }}>
                  ...
                  {(order.status === "DRAFT" || order.status === "BRIEFING") && !submitDone && (
                    <button onClick={requestHelp} disabled={helpRequested}
                      style={{ width: "100%", marginTop: 8, padding: "0.6em", borderRadius: 8, border: "1px solid var(--dash-accent-border)", background: helpRequested ? "var(--dash-accent-bg)" : "transparent", color: helpRequested ? "var(--dash-success)" : "var(--dash-accent)", fontSize: "0.82rem", fontWeight: 500, cursor: helpRequested ? "default" : "pointer", fontFamily: "inherit" }}>
                      <i className={`bx ${helpRequested ? "bx-check" : "bx-support"}`} style={{ marginRight: 6, verticalAlign: "middle" }} />
              ...
              <div className="dash-col2">
                ...
                {/* Progress */}
                {hasActiveStages && (
                  <DashProgressCard current={approved} total={total} className="dash-surface-card--mb" />
                )}
                ...
                {!order.specialist && order.status !== "DRAFT" && (
                  <div style={{ background: "var(--dash-surface)", borderRadius: 10, padding: 20, textAlign: "center", border: "1px solid var(--dash-border)", marginBottom: 16 }}>
                    <i className="bx bx-user-plus" style={{ fontSize: 32, color: "var(--dash-muted)", opacity: 0.4 }} />
                    <p style={{ color: "var(--dash-muted)", fontSize: "0.82rem", margin: "8px 0 0" }}>Специалист еще не назначен</p>
                  </div>
                )}
              </div>
            </div>
