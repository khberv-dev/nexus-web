    ...
    >
      ...
      <div style={{ fontSize: "0.82rem", lineHeight: 1.55, color: "var(--dash-text2)", marginBottom: 14 }}>
        <p style={{ margin: "0 0 8px" }}>
          ...
          <strong style={{ color: "var(--dash-text)" }}>без доплаты</strong> (каждый раунд — после согласования вы отправляете замечания,
          дизайнер дорабатывает материалы).
        </p>
        {!skipPayments ? (
          <p style={{ margin: 0 }}>
            Если лимит исчерпан, этап переходит в режим дополнительной оплаты: менеджер формирует начисление и при необходимости выставляет
            счёт. После успешной оплаты этап снова возвращается в работу. Оплатить дополнительные правки можно на этой странице (если есть
            начисление) или на карточке этапа; все счета и история платежей — в разделе{" "}
            <strong style={{ color: "var(--dash-text)" }}>«Оплата»</strong>.
          </p>
        ) : (
          <p style={{ margin: 0 }}>
            Оплата этапов в системе сейчас отключена — после исчерпания лимита процесс согласуется с менеджером отдельно.
          </p>
        )}
      </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center", marginBottom: 14 }}>
        <Link
          ...
          style={{ fontSize: "0.82rem", padding: "0.5em 1em", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6 }}
        >
      ...
      {openInvoices.length > 0 && (
        <div style={{ marginBottom: pendingExtraLines.length > 0 ? 14 : 0 }}>
          <p style={{ fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--dash-muted)", margin: "0 0 8px" }}>
              ...
              return (
                ...
                >
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: "0.76rem", color: "var(--dash-muted)", marginTop: 2 }}>{inv.purpose}</div>
      ...
      {!skipPayments && pendingExtraLines.length > 0 && (
        <div>
          <p style={{ fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--dash-danger)", margin: "0 0 8px" }}>
            ...
            {pendingExtraLines.map(({ stageId, stageType, ep }) => (
              ...
              >
                <div style={{ minWidth: 0 }}>
                  ...
                  {ep.reason ? (
                    <div style={{ fontSize: "0.76rem", color: "var(--dash-muted)", marginTop: 4 }}>{ep.reason}</div>
                  ) : null}
                  <div style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--dash-danger)", marginTop: 6 }}>
