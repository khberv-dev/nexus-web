...
import { isVideoFilename, clientFileTimeIso } from "./utils"
...
export function FileThumbnail({
  ...
  stageId: string
  file: { id: string; filename: string; createdAt: string; audience?: string }
  ...
  return (
    <div
      ...
      tabIndex={0}
      onClick={() => onClick(url)}
      onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onClick(url) } }}
      style={{
        width: 80,
        height: 80,
        borderRadius: 8,
      ...
      title={clientFileTimeIso(file) ? new Date(clientFileTimeIso(file)).toLocaleString("ru-RU") : undefined}
    >
      {isImage && (
        <img
          src={url}
