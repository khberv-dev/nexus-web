              ...
              >
                ...
                >
                  <div style={{ minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: "0.7rem", color: "var(--dash-muted)", margin: "4px 0 0", fontWeight: 500 }}>
                ...
                {archivedNotice ? (
                  <p style={{ fontSize: "0.75rem", color: "var(--dash-muted)", margin: "0 0 0.65rem", lineHeight: 1.45 }}>
                    {archivedNotice}
                  </p>
                ) : null}

                {w.moderatorRejections.length > 0 ? (
                  <div style={{ marginBottom: "0.65rem", paddingLeft: 10, borderLeft: "3px solid rgba(245, 158, 11, 0.9)" }}>
                    <p style={{ fontSize: "0.68rem", fontWeight: 600, color: "var(--dash-muted)", margin: "0 0 0.35rem" }}>
                ...
                {w.clientRejections.length > 0 ? (
                  <div style={{ marginBottom: "0.65rem", paddingLeft: 10, borderLeft: "3px solid rgba(56, 189, 248, 0.95)" }}>
                    <p style={{ fontSize: "0.68rem", fontWeight: 600, color: "var(--dash-muted)", margin: "0 0 0.35rem" }}>
                  ...
                  {vf.map((f, idx) => (
                    <div key={f.id}>
                      ...
                      >
                        ...
                        <span style={{ fontSize: "0.65rem", color: "var(--dash-muted)", marginLeft: 4, whiteSpace: "nowrap" }}>
                          {(() => {
        ...
        : revisionVariants.some((v) => v.files.length > 0) &&
          revisionVariants.map((variant, vIdx) => {
            ...
            return (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap", marginBottom: "0.45rem" }}>
                  <p style={{ fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--dash-muted)", margin: 0 }}>
                    {heading}
                  </p>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    {multiRound && isLastVariant && stage.status === "APPROVED" && (
                      <span style={{ fontSize: "0.72rem", fontWeight: 700, color: "var(--dash-success)", padding: "3px 10px", borderRadius: 999, background: "rgba(46,184,92,0.12)", border: "1px solid var(--dash-success)" }}>
                        Принято — финальная версия
                      </span>
                    )}
                    {multiRound && isLastVariant && stage.status === "CLIENT_REVIEW" && (
                      <span style={{ fontSize: "0.72rem", fontWeight: 700, color: "var(--dash-warn)", padding: "3px 10px", borderRadius: 999, background: "var(--dash-warn-bg)", border: "1px solid var(--dash-warn)" }}>
                        На согласовании
                      </span>
                    )}
                    {multiRound && isLastVariant && stage.status === "CLIENT_REVISION" && (
                      <span style={{ fontSize: "0.72rem", fontWeight: 700, color: "var(--dash-muted)", padding: "3px 10px", borderRadius: 999, background: "var(--dash-surface2)", border: "1px solid var(--dash-border)" }}>
                        Доработка у дизайнера
                      </span>
                    )}
                  </div>
                ...
                {variant.revisionFeedback ? (
                  <div style={{ marginBottom: "0.65rem", padding: "0.55rem 0.65rem", borderRadius: 8, background: "var(--dash-surface2)", borderLeft: "3px solid var(--dash-warn)" }}>
                    <p style={{ fontSize: "0.68rem", fontWeight: 600, color: "var(--dash-muted)", margin: "0 0 0.25rem" }}>
                      Замечания к этой версии
                    </p>
                    <p style={{ fontSize: "0.82rem", color: "var(--dash-text)", margin: 0, whiteSpace: "pre-wrap" }}>{variant.revisionFeedback}</p>
                  </div>
                ) : null}
