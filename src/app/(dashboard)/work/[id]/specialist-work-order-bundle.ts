...
export type SpecialistWorkOrderBundle = {
  ...
  order: {
    ...
    stages: Array<{
      ...
      files: Array<{ id: string; filename: string; createdAt: string; hasAnnotations?: boolean; audience?: "DESIGNER" | "CLIENT" | "SHARED" }>
...
export async function loadSpecialistWorkOrderBundle(orderId: string, specialistUserId: string): Promise<SpecialistWorkOrderBundle | null> {
  const order = await prisma.order.findFirst({
    ...
    include: {
      ...
      stages: {
        orderBy: { type: "asc" },
        include: {
          files: { orderBy: { uploadedAt: "desc" }, select: { id: true, filename: true, uploadedAt: true, annotations: true, audience: true } },
          reviews: { orderBy: { createdAt: "desc" }, take: 25, select: { id: true, reviewerRole: true, verdict: true, comment: true, createdAt: true } },
          act: {
            select: {
              id: true,
              signedAt: true,
  ...
  const pipelineStages: PipelineStage[] = stagesSorted.map(s => ({
    ...
    act: s.act
      ...
      : null,
    extraPayments: s.extraPayments.map(ep => ({ id: ep.id, amount: ep.amount, reason: ep.reason, status: ep.status })),
  }))
