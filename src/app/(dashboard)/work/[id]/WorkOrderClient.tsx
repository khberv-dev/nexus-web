        ...
        <div onClick={onClose} style={{
            ...
            >
                ...
                {showMarkupViewer ? (
                    ...
                    <>
                        {isImage && <img src={url} alt={filename} style={{
                            maxWidth: "85vw",
                            maxHeight: "85vh",
                            objectFit: "contain",
                            borderRadius: 8,
                        ...
                        {isVideo && <video src={url} controls autoPlay style={{
                            maxWidth: "85vw",
                            maxHeight: "85vh",
                            borderRadius: 8,
                            display: "block"
...
export default function WorkOrderClient({
    ...
    const openChat = useCallback(() => openOrderChat(initial.id, {
        focus: false,
                ...
                {!focusedStageType ? (
                    <>
                        <div style={{marginBottom: 20}}>
                            <DashPageTitle subtitle={new Date(order.createdAt).toLocaleDateString("ru-RU", {
                                day: "numeric",
                                month: "long",
                                year: "numeric"
                            })}>
                        ...
                        <div className="dash-content">
                            <div className="dash-col1">
                                ...
                                {briefHelpRequested && (
                                    <DashSurfaceCard padding="md" className="dash-surface-card--mb" style={{
                                        borderColor: "var(--dash-warn)",
                        ...
                        Заказчик запросил помощь менеджера по брифу — учтите это при изучении материалов и уточняющих вопросах.
                                ...
                                <ClientContractPanel
                                    ...
                                    onUploadSigned={async (file) => {
                                        ...
                                        fd.append("file", file)
                                        const res = await fetch(`/api/orders/${order.id}/contract/specialist/sign`, {
                                            method: "POST",
                                            body: fd
                                        })
                                        if (res.ok) {
                          ...
                          {p.status === "PENDING" ? "Ожидает" : p.status === "HELD" ? "Удержана" : p.status === "RELEASED" ? "Выплачена" : p.status}
                                ...
                                {order.briefData && Object.values(order.briefData).some(Boolean) && (
                                    <div id="order-brief" className="dash-surface-card--mb"
                                         style={{scrollMarginTop: 88, marginTop: 16}}>
                                        <DashBriefCard title="Бриф проекта" labels={SPECIALIST_ORDER_BRIEF_LABELS}
                                                       values={order.briefData} showOnlyFilled/>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="dash-col2">
                                <div className="dash-order-top">
                                    <DashInfoChip dot="A" label="Заказчик"
                                                  value={order.client.name ?? order.client.email}/>
                                ...
                                {focusedStage ? (
                                    <div className="dash-brief-row"
                                        ...
                                        <span className="dash-brief-value"
                                              style={{color: STAGE_STATUS_SHARED[focusedStage.status as StageStatus]?.color}}>
                      {stageStatusLabel}
                    </span>
                                    </div>
                                ) : null}
                        ...
                        <div className="dash-col2 order-work-stage-col2">
                            <div className="order-work-stage-main">
                              ...
                              <span style={{
                                  ...
                                  flexShrink: 0
                                                        ...
                                                        <span className="dash-stage-card__status"
                                                              style={{color: STAGE_STATUS_SHARED[focusedStage.status as StageStatus]?.color ?? "var(--dash-muted)"}}>
                              {specialistStageStatusLabel(focusedStage.type, focusedStage.status)}
                            </span>
                                                    )}
                                                </div>
                            ...
                            {focusedStage ? (
                                <aside className="order-work-history-aside" aria-label="История этапа">
                                    <OrderHistoryTimeline orderId={order.id} stageId={focusedStage.id}
                                                          viewerRole="SPECIALIST"/>
                                </aside>
                            ) : null}
                        </div>
