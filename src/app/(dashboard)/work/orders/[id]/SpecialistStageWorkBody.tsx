...
function FileThumbnail({stageId, file, onClick}: {
    ...
    return (
        <div role="button" tabIndex={0} onClick={() => onClick(url)} onKeyDown={(e) => {
...
const ACT_STATUS_LABEL: Record<string, { label: string; icon: string; color: string }> = {
    ...
    CLIENT_SIGNED: {
        label: "Подписан заказчиком, ожидает подтверждения",
...
function ActSection({stage, onUploadAct}: {
    ...
    const statusInfo = ACT_STATUS_LABEL[act.status] ?? {
        label: act.status,
        ...
        <>
            <div style={{display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10}}>
                {([
                    ...
                    {
                        key: "fixes",
                        label: annotatedFiles.length > 0 ? `Правки (пометки) · ${annotatedFiles.length}` : "Правки (пометки)"
                    },
                ] as const).map((t) => {
                                ...
                                <div style={{marginTop: 10}}>
                                    {firstSendMedia.length > 0 ? (
                                        <div style={{
                                            ...
                                            gap: 8,
                                            marginBottom: firstSendDocs.length > 0 ? 8 : 0
                                        }}>
                                    ...
                                    >
                                        <div style={{
                                            ...
                                            <div style={{minWidth: 0}}>
                                                <p className="dash-file-list__title"
                                                   style={{marginBottom: waveMulti ? 0 : 4}}>
                                                    {heading}
                                                </p>
                                                <p style={{
                                                    fontSize: "0.7rem",
                        ...
                        <div style={{
                            ...
                            {stage.extraPayments.map(ep => (
                                <div key={ep.id} style={{
                                    ...
                                    <span style={{
                                        fontWeight: 600,
                                        color: ep.status === "PENDING" ? "var(--dash-warn)" : ep.status === "HELD" ? "var(--dash-accent)" : "var(--dash-success)"
                                    }}>
