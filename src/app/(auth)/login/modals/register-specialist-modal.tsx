...
import {LEGAL_PERSONAL_DATA_HREF, SPECIALIST_REGISTER_FIELDS} from "../constants"
...
export function RegisterSpecialistModal({
                                            open,
                                            onClose,
                                            regForm,
                                            setRegForm,
    ...
    return (
        <AuthGlassModal open={open} onClose={onClose} maxWidth={440}>
            <h2 style={{margin: "0 2.25rem 1rem 0", fontSize: "1.2rem", fontWeight: 500}}>Регистрация дизайнера</h2>
            <form onSubmit={onSubmit}>
                {SPECIALIST_REGISTER_FIELDS.map((field) => (
                    <div key={field.name} style={{marginBottom: 12}}>
              <span style={{display: "block", marginBottom: 6, fontSize: "0.76rem", color: "rgba(255,255,255,0.45)"}}>
                {field.label}
                  {field.req && <span style={{color: "#f87171", marginLeft: 3}}>*</span>}
              </span>
                        <input
                            type={field.type}
                            required={field.req}
