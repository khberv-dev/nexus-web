...
export async function GET() {
  ...
  try {
    ...
    if (step.status === "IN_PROGRESS" && data.answers) {
      return NextResponse.json({ resume: { answers: data.answers, score: data.score ?? 0, sectionScores: data.sectionScores ?? {} } })
    }
  } catch { /* ignore */ }
...
export async function POST(req: NextRequest) {
  ...
  const comment = JSON.stringify({ score, total, pct, passed, sectionScores, answers, finishedAt: new Date().toISOString() })
