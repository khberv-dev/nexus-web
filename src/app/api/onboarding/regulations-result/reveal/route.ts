...
export async function POST(req: NextRequest) {
  ...
  let prevData: { answers?: Record<string, number>; score?: number; sectionScores?: Record<string, { correct: number; total: number }> } = {}
  if (existing?.status === "IN_PROGRESS" && existing.comment) {
    try { prevData = JSON.parse(existing.comment) } catch { /* ignore */ }
  }
  ...
  if (isCorrect) sectionScores[q.section] = { ...sectionScores[q.section], correct: (sectionScores[q.section]?.correct ?? 0) + 1 }
