...
async function authorizedFile(stageId: string, fileId: string, userId: string, role: string) {
  ...
  if (role === "SPECIALIST" && order.specialistId !== userId) return { error: "Forbidden" as const, status: 403 as const }
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string; fid: string }> }) {
  ...
  if ("error" in res) {
    if (res.status === 404) console.warn("[annotations] StageFile not found", { stageId, fid, role: user.role, userId: user.id })
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string; fid: string }> }) {
  ...
  if ("error" in gate) {
    if (gate.status === 404) console.warn("[annotations] StageFile not found (PATCH)", { stageId, fid, role: user.role, userId: user.id })
