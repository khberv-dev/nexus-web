...
const CLIENT_HISTORY_OR: Prisma.AuditLogWhereInput[] = [
    {
        entity: "Order",
        action: {in: ["brief_updated", "brief_submitted", "specialist_assigned", "order_status_changed", "framework_contract_admin_signed"]}
    },
    {
        entity: "Contract",
...
export async function GET(req: NextRequest, {params}: { params: Promise<{ id: string }> }) {
    ...
    function anchorFor(log: (typeof page)[0]): Anchor | null {
        ...
        if (log.entity === "Order") {
            const a = log.action
            if (a === "brief_updated" || a === "brief_submitted") return {
                kind: "path",
                path: `/orders/${orderId}`,
                hash: "#order-brief"
            }
            if (a === "specialist_assigned" || a === "order_status_changed") return {
                kind: "path",
                path: `/orders/${orderId}`,
                hash: "#order-brief"
            }
            if (a === "framework_contract_admin_signed") return {
                kind: "path",
                path: `/orders/${orderId}`,
                hash: "#order-contract"
            }
