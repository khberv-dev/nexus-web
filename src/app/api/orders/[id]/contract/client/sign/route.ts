...
/**
 * POST /api/orders/[id]/contract/client/sign
 * Заказчик загружает подписанный скан договора
...
export async function POST(req: NextRequest, {params}: { params: Promise<{ id: string }> }) {
    ...
    for (const admin of admins) {
        void notify(
            ...
            "Договор подписан заказчиком",
            `Заказчик подписал договор ${contract.number} по заказу #${orderId}. Подтвердите для активации.`,
