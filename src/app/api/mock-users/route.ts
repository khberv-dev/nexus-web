  ...
  const mapped = await Promise.all(
    users.map(async u => {
      ...
      if (u.files[0]) {
        ...
        if (file) try { avatarUrl = (await getDownloadUrl(file.s3Key)).url } catch {}
      }

      return { id: u.id, email: u.email, name: fd?.fullName ?? u.name ?? null, zitadelId: u.zitadelId, role: u.role, href, avatarUrl }
    })
  )
