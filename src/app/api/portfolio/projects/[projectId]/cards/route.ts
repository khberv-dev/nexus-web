...
import { validateAttachmentSpecsForNewCard, type AttachmentCreateSpec } from "@/lib/portfolioCreateAttachments"
  ...
  const attachmentSpecs: AttachmentCreateSpec[] | undefined = (() => {
    if (body.attachmentSpecs?.length) {
      ...
      for (const s of body.attachmentSpecs) {
        if (s?.fileId) byFile.set(s.fileId, { fileId: s.fileId, linkedVisualFileId: s.linkedVisualFileId ?? null })
      }
