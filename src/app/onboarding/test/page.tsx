...
import {
  type DecodedQuizQuestion,
  type NexusQuizQuestionWire,
  decodeQuizQuestionWire,
} from "@/lib/onboarding/quiz-wire"
...
export default function OnboardingTestPage() {
  ...
  const [resultFail, setResultFail] = useState<{ correctCount: number; total: number; percent: number; passPercent: number; attemptsLeft: number } | null>(
    null
  )
  const [resultOk, setResultOk] = useState<{ percent: number; transitionText?: string } | null>(null)
  const [resultExhausted, setResultExhausted] = useState<{ level: QuizLevelCode; onboardingStatus: string; comment?: string } | null>(null)
  ...
  const finishQuiz = useCallback(async () => {
    ...
    setPhase("result")
    setResultOk({ percent: Number(data.percent), transitionText: typeof data.transitionText === "string" ? data.transitionText : undefined })
        ...
        {gate === "ok" && payload && phase === "intro" && (
          <>
            ...
            <div className="flex flex-wrap gap-8 mb-8" style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.85rem" }}>
              <div>
                <div style={{ color: "#f4f4f4", fontSize: "1.75rem", fontWeight: 600, lineHeight: 1 }}>{total}</div>
                <div>вопросов</div>
              </div>
              <div>
                <div style={{ color: "#f4f4f4", fontSize: "1.75rem", fontWeight: 600, lineHeight: 1 }}>{passPercent}%</div>
                <div>проходной балл</div>
              </div>
              <div>
                <div style={{ color: "#f4f4f4", fontSize: "1.75rem", fontWeight: 600, lineHeight: 1 }}>{payload.level}</div>
                <div>уровень</div>
              </div>
            </div>
            {payload.resume && payload.resume.answeredCount > 0 ? (
              <div className="flex flex-col gap-3">
                <AppCard>
                  <p style={{ color: "rgba(255,255,255,0.75)", fontSize: "0.88rem", margin: 0, lineHeight: 1.5 }}>
                    Сохранен прогресс: отвечено <strong style={{ color: "#f4f4f4" }}>{payload.resume.answeredCount}</strong> из{" "}
                    {total}. Верных на данный момент (по сохраненным ответам):{" "}
                    <strong style={{ color: "#f4f4f4" }}>{payload.resume.liveCorrect}</strong>.
                  </p>
                  <p style={{ color: "rgba(255,255,255,0.35)", fontSize: "0.78rem", margin: "0.75em 0 0", lineHeight: 1.45 }}>
        ...
        {gate === "ok" && phase === "quiz" && current && (
          <>
            ...
            <AppCard key={current.id}>
              ...
              {serverError && (
                <p style={{ color: "#fca5a5", fontSize: "0.82rem", marginTop: "0.75rem" }}>{serverError}</p>
              )}
        ...
        {phase === "result" && resultFail && (
          <AppCard>
            ...
            <p style={{ color: "rgba(255,255,255,0.55)", fontSize: "0.9rem", lineHeight: 1.5, margin: 0 }}>
              Набрано {resultFail.correctCount} из {resultFail.total} ({resultFail.percent}%). Необходимо минимум {resultFail.passPercent}%.
