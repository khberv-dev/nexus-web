...
const STEPS = [
  { key: "FORM",        label: "Анкета",      desc: "Заполните информацию о себе и опыте работы",          href: "/onboarding/form", action: "Заполнить анкету →" },
  { key: "TEST",        label: "Квалификационный тест",         desc: "Открывается после приглашения администратора", href: "/onboarding/test", action: "Пройти тест →" },
  { key: "INTERVIEW",   label: "Интервью",     desc: "Zoom-интервью с командой платформы",                  href: "/onboarding/interview", action: "Перейти к интервью →" },
  { key: "REGULATIONS_READ", label: "Регламент",   desc: "Ознакомьтесь с правилами работы на платформе",       href: "/onboarding/regulations/read", action: "Ознакомиться →" },
  { key: "REGULATIONS", label: "Тест по регламенту",   desc: "Проверьте знание правил платформы",       href: "/onboarding/regulations", action: "Пройти тест →" },
  { key: "CONTRACT",    label: "Договор",      desc: "Подписание договора с платформой",                    href: "/onboarding/contract", action: "Подписать договор →" },
]
      ...
      <div className="mx-auto max-w-2xl px-6 py-12">
        ...
        <div className="flex flex-col gap-3">
          {STEPS.map((step, i) => {
            ...
            return (
              <AppCard key={step.key} style={{ opacity: isPending ? 0.45 : 1, cursor: isPending ? "default" : "auto" }}>
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between gap-3">
                      <span style={{ color: "#f4f4f4", fontSize: "0.95rem", fontWeight: 500 }}>{step.label}</span>
                      <StatusBadge
                        variant={isDone ? "done" : isCurrent ? "current" : "pending"}
                        label={isDone ? "Готово" : isCurrent ? "Текущий" : "Ожидает"}
                      />
                    </div>
                    <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.82rem", marginTop: "0.35em" }}>{step.desc}</p>
                    ...
                    {isCurrent && (step.key === "TEST" && !testUnlocked) && (
                      <p style={{ color: "rgba(255,255,255,0.3)", fontSize: "0.8rem", marginTop: "0.5em", fontStyle: "italic" }}>
                        Ожидайте подтверждения анкеты администратором. После этого откроется доступ к тесту.
                      </p>
                    )}
        ...
        {allDone && (
          <div style={{ marginTop: "2rem", display: "flex", flexDirection: "column", gap: 12 }}>
            <AppCard style={{ background: "rgba(52,211,153,0.07)", border: "1px solid rgba(52,211,153,0.2)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(52,211,153,0.15)", border: "1px solid rgba(52,211,153,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1rem", flexShrink: 0 }}>✓</div>
                <div>
                  <div style={{ color: "#34d399", fontWeight: 500, fontSize: "0.95rem" }}>Верификация пройдена</div>
                  <div style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.82rem", marginTop: 2 }}>Все этапы успешно завершены. Вы можете приступать к работе.</div>
                </div>
              </div>
            </AppCard>
            <Link
