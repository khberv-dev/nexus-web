    ...
    <OnboardingShell title="Регламент" backHref="/onboarding" backLabel="Онбординг" withBg>
      <div className="mx-auto max-w-3xl px-6 py-12">
        <div className="mb-8">
          ...
          <p style={{ color: "rgba(255,255,255,0.45)", marginTop: "0.5em", fontSize: "0.9rem", lineHeight: 1.55 }}>
            Перед тестом прочитайте регламент платформы. Пролистайте текст до конца и подтвердите ознакомление.
          </p>
        ...
        <AppCard>
          ...
          >
            {REGULATION_SECTIONS.map((s, idx) => (
              <div key={idx} style={{ marginBottom: 18 }}>
                ...
                <div style={{ color: "rgba(255,255,255,0.72)", fontSize: "0.88rem", lineHeight: 1.65, whiteSpace: "pre-wrap" }}>
                  {s.body}
                </div>
              </div>
            ))}
          ...
          <div style={{ marginTop: 14, borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 14 }}>
            <label style={{ display: "flex", gap: 10, alignItems: "flex-start", cursor: scrolledToEnd ? "pointer" : "not-allowed" }}>
              <input
                type="checkbox"
                checked={confirmed}
                ...
                style={{ marginTop: 3 }}
              />
              <span style={{ color: scrolledToEnd ? "rgba(255,255,255,0.75)" : "rgba(255,255,255,0.35)", fontSize: "0.85rem", lineHeight: 1.45 }}>
                Я ознакомился(ась) с регламентом и обязуюсь соблюдать правила платформы.
                {!scrolledToEnd && (
                  <span style={{ display: "block", marginTop: 4, color: "rgba(255,255,255,0.28)", fontSize: "0.8rem" }}>
