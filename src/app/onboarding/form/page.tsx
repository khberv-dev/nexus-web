...
const FIELDS = [
  ...
  { name: "specialty", label: "Специализация", type: "text", placeholder: "Коммерческие интерьеры, офисы", required: false },
  ...
  { name: "software", label: "Программы", type: "software", placeholder: "Начните вводить или выберите ниже…", required: false },
  { name: "about", label: "О себе", type: "textarea", placeholder: "Расскажите о вашем опыте и специализации...", required: true },
]
...
export default function OnboardingFormPage() {
  ...
  const lookupInn = async (inn: string) => {
    ...
    if (isIpInn || isOooInn) {
      try {
        const res = await fetch("/api/dadata/party", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ inn: cleanInn }) })
  ...
  const lookupBik = async (bik: string) => {
    ...
    if (bik.replace(/\D/g, "").length === 9) {
      try {
        const res = await fetch("/api/dadata/bank", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ bik: bik.replace(/\D/g, "") }) })
        const data = await res.json()
    ...
    <OnboardingShell title="Анкета" backHref="/onboarding" backLabel="Онбординг" withBg>
      <div className="mx-auto max-w-xl px-6 py-12">
        ...
        <AppCard>
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                ...
                return (
                  ...
                  >
                    ...
                    {field.type === "toggle" ? (
                      <button
                        type="button"
                        onClick={() => setForm(f => ({ ...f, [field.name]: f[field.name] === "true" ? "false" : "true" }))}
                        style={{
                          padding: "0.5em 1.2em", borderRadius: 8, fontSize: "0.85rem", cursor: "pointer", fontFamily: "inherit",
            ...
            <div className="flex flex-col gap-3" style={{ marginTop: 8, padding: "1rem", background: "rgba(255,255,255,0.03)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.08)" }}>
              <label style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.8rem", fontWeight: 500 }}>Налоговый статус (обязательно)</label>
              <div style={{ display: "flex", gap: "0.5rem" }}>
                {TAX_STATUSES.map(s => (
                  <button key={s.value} type="button" onClick={() => setForm(f => ({ ...f, taxStatus: s.value }))} style={{
                    flex: 1, padding: "0.7em", borderRadius: 8, cursor: "pointer", fontFamily: "inherit", textAlign: "center",
              ...
              {form.taxStatus === "SZ" && (
                <div style={{ padding: "8px 12px", borderRadius: 8, background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.15)", fontSize: "0.78rem", color: "rgba(255,255,255,0.6)" }}>
                  <i className="bx bx-info-circle" style={{ marginRight: 4, color: "#6366f1" }} />
                  Самозанятый формирует чеки в приложении «Мой налог» после каждой выплаты. Платформа является агентом.
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      onFocus={e => (e.target.style.borderColor = "rgba(255,255,255,0.35)")} onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
                    <div style={{ fontSize: "0.72rem", color: "rgba(255,255,255,0.38)", marginTop: 4 }}>
                      Подтянем данные автоматически после ввода ИНН.
                    </div>
                    {form.taxStatus === "IP" && form.ipName && <div style={{ fontSize: "0.75rem", color: "#34d399", marginTop: 2 }}>{form.ipName}</div>}
                    {form.taxStatus === "OOO" && form.companyName && <div style={{ fontSize: "0.75rem", color: "#34d399", marginTop: 2 }}>{form.companyName}</div>}
                  </div>
                  {form.taxStatus === "IP" && (
                    <>
                      <div className="flex flex-col gap-1.5">
                        <label style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.8rem", fontWeight: 500 }}>Наименование ИП</label>
                        <input type="text" value={form.ipName || ""} onChange={e => setForm(f => ({ ...f, ipName: e.target.value }))} style={inputStyle} placeholder="ИП Иванов Иван Иванович"
                          onFocus={e => (e.target.style.borderColor = "rgba(255,255,255,0.35)")} onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
                      </div>
                      <div className="flex flex-col gap-1.5">
                        <label style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.8rem", fontWeight: 500 }}>ОГРНИП</label>
                        <input type="text" value={form.ogrnip || ""} onChange={e => setForm(f => ({ ...f, ogrnip: e.target.value }))} style={inputStyle} maxLength={15} placeholder="304770000000000"
                          onFocus={e => (e.target.style.borderColor = "rgba(255,255,255,0.35)")} onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
                      </div>
                      <div className="flex flex-col gap-1.5">
                        <label style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.8rem", fontWeight: 500 }}>Дата регистрации ИП</label>
                        <input type="text" value={form.ipRegDate || ""} onChange={e => setForm(f => ({ ...f, ipRegDate: e.target.value }))} style={inputStyle} placeholder="01.01.2020"
                          onFocus={e => (e.target.style.borderColor = "rgba(255,255,255,0.35)")} onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
                      </div>
                    </>
                  )}
                  {form.taxStatus === "OOO" && (
                    <>
                      <div className="flex flex-col gap-1.5">
                        <label style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.8rem", fontWeight: 500 }}>Наименование ООО</label>
                        <input type="text" value={form.companyName || ""} onChange={e => setForm(f => ({ ...f, companyName: e.target.value }))} style={inputStyle} placeholder="ООО «Пространство»"
                          onFocus={e => (e.target.style.borderColor = "rgba(255,255,255,0.35)")} onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
                      </div>
                      <div className="flex flex-col gap-1.5">
                        <label style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.8rem", fontWeight: 500 }}>КПП</label>
                        <input type="text" value={form.kpp || ""} onChange={e => setForm(f => ({ ...f, kpp: e.target.value }))} style={inputStyle} maxLength={9} placeholder="770701001"
                          onFocus={e => (e.target.style.borderColor = "rgba(255,255,255,0.35)")} onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
                      </div>
                      <div className="flex flex-col gap-1.5">
                        <label style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.8rem", fontWeight: 500 }}>ОГРН</label>
                        <input type="text" value={form.ogrn || ""} onChange={e => setForm(f => ({ ...f, ogrn: e.target.value }))} style={inputStyle} maxLength={13} placeholder="1027700132195"
                          onFocus={e => (e.target.style.borderColor = "rgba(255,255,255,0.35)")} onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
                      </div>
                      <div className="flex flex-col gap-1.5" style={{ gridColumn: "1 / -1" }}>
                        <label style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.8rem", fontWeight: 500 }}>Юридический адрес</label>
                        <input type="text" value={form.legalAddress || ""} onChange={e => setForm(f => ({ ...f, legalAddress: e.target.value }))} style={inputStyle} placeholder="г. Москва, ул. Примерная, д. 1"
                          onFocus={e => (e.target.style.borderColor = "rgba(255,255,255,0.35)")} onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
                      </div>
                    </>
                  )}
                  <div className="flex flex-col gap-1.5">
                    <label style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.8rem", fontWeight: 500 }}>{form.taxStatus === "SZ" ? "Счет карты / р/с" : "Расчетный счет"}</label>
                    <input type="text" value={form.bankAccount || ""} onChange={e => setForm(f => ({ ...f, bankAccount: e.target.value }))} style={inputStyle} maxLength={20} placeholder="40802810000000000000"
                      onFocus={e => (e.target.style.borderColor = "rgba(255,255,255,0.35)")} onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.8rem", fontWeight: 500 }}>Банк</label>
                    <input type="text" value={form.bankName || ""} onChange={e => setForm(f => ({ ...f, bankName: e.target.value }))} style={inputStyle} placeholder="АО «Т-Банк»"
                      onFocus={e => (e.target.style.borderColor = "rgba(255,255,255,0.35)")} onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.8rem", fontWeight: 500 }}>БИК</label>
                    <input type="text" value={form.bankBik || ""} onChange={e => lookupBik(e.target.value)} style={highlightedAutoFillInputStyle} maxLength={9} placeholder="044525974"
                      onFocus={e => (e.target.style.borderColor = "rgba(255,255,255,0.35)")} onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
                    <div style={{ fontSize: "0.72rem", color: "rgba(255,255,255,0.38)", marginTop: 4 }}>
                  ...
                  {form.taxStatus === "OOO" && (
                    <div className="flex flex-col gap-1.5">
                      <label style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.8rem", fontWeight: 500 }}>Корр. счет</label>
                      <input type="text" value={form.corrAccount || ""} onChange={e => setForm(f => ({ ...f, corrAccount: e.target.value }))} style={inputStyle} maxLength={20} placeholder="30101810000000000000"
                        onFocus={e => (e.target.style.borderColor = "rgba(255,255,255,0.35)")} onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
                    </div>
                  )}
                </div>
            ...
            {error && (
              <div style={{ color: "#f87171", fontSize: "0.85rem", background: "rgba(240,20,20,0.1)", border: "1px solid rgba(240,20,20,0.2)", borderRadius: 8, padding: "0.6em 1em" }}>
                {error}
              </div>
            )}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                {[1, 2, 3].map(i => (
                  <div key={i} style={{
                    ...
                    <div style={{ background: "rgba(255,255,255,0.07)", borderRadius: 4, height: 10, width: "40%", marginBottom: 10 }} />
                    <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 4, height: 8, width: "85%", marginBottom: 8 }} />
                    <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 4, height: 8, width: "65%" }} />
                  </div>
                ))}
                <p style={{ color: "rgba(255,255,255,0.25)", fontSize: "0.8rem", textAlign: "center", margin: "0.5rem 0 0" }}>
                  Анализирую анкету…
                </p>
              </div>
            )}
            ...
            {!loadingAI && !aiError && suggestions.map((s, i) => {
              ...
              return (
                ...
                >
                  ...
                  <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.8rem", margin: "0 0 0.875em" }}>
                    {s.reason}
                  </p>
                  <div style={{
                    ...
                    <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.8rem", fontStyle: "italic", margin: 0 }}>
                      «{s.example}»
                    </p>
                  </div>
      ...
      {videoOpen && (
        <div onClick={() => setVideoOpen(false)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 640, borderRadius: 16, overflow: "hidden", background: "#000", position: "relative" }}>
            <button onClick={() => setVideoOpen(false)} style={{
              position: "absolute", top: 12, right: 12, zIndex: 10, width: 32, height: 32, borderRadius: "50%",
              background: "rgba(255,255,255,0.15)", border: "none", color: "#fff", cursor: "pointer", fontSize: "1rem",
              display: "flex", alignItems: "center", justifyContent: "center",
            }}><i className="bx bx-x" /></button>
            <video src="/sz/payment_agents.mp4" controls autoPlay playsInline style={{ width: "100%", display: "block" }} />
          </div>
        </div>
      )}
