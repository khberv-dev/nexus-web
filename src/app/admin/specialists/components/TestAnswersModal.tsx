...
import type { QuizLevelCode, QuizLevelAttempt } from "@/lib/onboarding/levels/types"
...
function parseState(comment: string | null | undefined): ParsedState | null {
  ...
  try {
    const p = JSON.parse(comment) as { version?: number; attempts?: QuizLevelAttempt[]; passedLevels?: QuizLevelCode[] }
...
export function TestAnswersModal({ testModal, onClose }: Readonly<{ testModal: TestModalData | null; onClose: () => void }>) {
      ...
      <div className="sp-modal-body">
        ...
        {attempts.length === 0 ? (
          <div className="sp-modal-empty">
            ...
            <p className="sp-modal-empty__sub">Специалист ещё не проходил тест или данные в старом формате.</p>
          </div>
        ) : (
          <>
            {/* Summary per level */}
              ...
              {QUIZ_LEVEL_ORDER.map(code => {
                ...
                return (
                  <div key={code} className="sp-card" style={{ textAlign: "center", padding: 8 }}>
                    ...
                    <div style={{ fontSize: "0.78rem", color: "var(--adm-muted)" }}>{LEVEL_LABELS[code]}</div>
                    {la.length > 0 ? (
                      <>
                        <span className={badgeClass} style={{ marginTop: 4 }}>{passed ? "✓ Сдан" : "✗ Не сдан"}</span>
                        <div style={{ fontSize: "0.7rem", color: "var(--adm-muted)", marginTop: 2 }}>
                          {la.length}/3 · {best}%
                        </div>
                      </>
                    ) : (
            ...
            {detailQuestions.length > 0 && Object.keys(detailAnswers).length > 0 && (
              <div style={{ marginTop: 16 }}>
                <div className="sp-label" style={{ marginBottom: 8 }}>
                  Ответы по вопросам{detailLevel ? ` (${detailLevel} — ${LEVEL_LABELS[detailLevel]})` : ""}
                </div>
                <div
                  style={{
                    maxHeight: 280,
        ...
        <div className="sp-modal-footer">
          ...
          <button className="sp-btn sp-btn-ghost" onClick={onClose} style={{ marginLeft: "auto" }}>Закрыть</button>
        </div>
      </div>
    </Modal>
  )
