    ...
    <>
      <div className="sp-onboarding-bar">
        <div className="sp-onboarding-bar__track">
          <div className="sp-onboarding-bar__fill" style={{ width: `${(doneCount / ONBOARDING_STEPS_UI.length) * 100}%` }} />
        </div>
        <div className="sp-onboarding-steps">
          {ONBOARDING_STEPS_UI.map((step, i) => {
            ...
            return (
              ...
              >
                <div className="sp-onboarding-step__dot">{done ? <i className="bx bx-check" /> : <span>{i + 1}</span>}</div>
                <span className="sp-onboarding-step__label">
                  {step.label}
                  {testReviewable && <i className="bx bx-show" style={{ marginLeft: 3, fontSize: "0.7rem" }} />}
                </span>
              </button>
            )
          })}
      ...
      {(testProgressLive || pendingApprovalLevel) && (
        <div className="sp-quiz-micro-wrap">
          <div className="sp-quiz-micro-label">
            {testProgressLive ? (
              <>
                Квалификационный тест: отвечено {testProgressLive.answeredCount} из {testProgressLive.total}
                {" · "}
          ...
          {testProgressLive && (
            <div className="sp-quiz-micro-ticks" aria-label="Прогресс по вопросам: зеленый — верно, красный — неверно">
              {Array.from({ length: testProgressLive.total }, (_, i) => {
                const { state, tooltip } = getQuizMicroTick(i, testProgressLive.answers, levelQuestionsForTicks)
