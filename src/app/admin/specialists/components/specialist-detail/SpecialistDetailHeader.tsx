    ...
    <div className="sp-detail-sticky">
      <div className="sp-profile-header">
        <div className="sp-av-xl">
          {avatarUrl
            ? <ImageLightbox src={avatarUrl} alt="Аватар"><img src={avatarUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /></ImageLightbox>
        ...
        <div className="sp-profile-info">
          ...
          <div className="sp-profile-meta">
            ...
            {sp.files.length > 0 && (
              <span className="sp-badge"><i className="bx bx-paperclip" style={{ marginRight: 3 }} />{sp.files.length} файл(ов)</span>
            )}
          </div>
          {fd?.city && (
        ...
        <div className="sp-profile-right">
          <div className="sp-profile-actions">
            {canAdvance && (
              <>
                <button onClick={() => onAct(sp.id, "advance")} disabled={acting !== null} className="sp-btn sp-btn-primary">
                  {acting === sp.id + "advance" ? "..." : ADVANCE_LABEL[status]}
                </button>
                {status === "REGULATIONS" && regulationsStepStatus !== "PASSED" && (
                  <span style={{ fontSize: "0.75rem", color: "rgba(255,200,100,0.85)", alignSelf: "center" }}>
            ...
            {canReject && status !== "PENDING" && (
              <button onClick={() => onAct(sp.id, "reject")} disabled={acting !== null} className="sp-btn sp-btn-danger">
      ...
      <div className="sp-detail-tabs">
        <button type="button" className={`sp-detail-tab${detailTab === "main" ? " sp-detail-tab--active" : ""}`} onClick={() => setDetailTab("main")}>
          Основной
        </button>
        <button
          type="button"
        ...
        <button type="button" className={`sp-detail-tab${detailTab === "files" ? " sp-detail-tab--active" : ""}`} onClick={() => setDetailTab("files")}>
          Файлы
        </button>
        <button type="button" className={`sp-detail-tab${detailTab === "portfolio" ? " sp-detail-tab--active" : ""}`} onClick={() => setDetailTab("portfolio")}>
          Портфолио
        </button>
        <button type="button" className={`sp-detail-tab${detailTab === "orders" ? " sp-detail-tab--active" : ""}`} onClick={() => setDetailTab("orders")}>
          Заказы
        </button>
      </div>
    </div>
