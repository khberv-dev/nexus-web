...
function AdminCoverThumb({ file }: { file: PortfolioFile }) {
  ...
  if (!isImageFile(file) || failed || !url) {
    return (
      ...
      >
        <i className="bx bx-image" style={{ fontSize: 36, opacity: 0.35, color: "var(--adm-muted)" }} aria-hidden />
      </div>
    )
  }
  ...
  if (err) {
    return (
      <div className="sp-warn" style={{ marginBottom: 0 }}>
        {err}
        <button type="button" className="sp-btn sp-btn-ghost" style={{ marginLeft: 10 }} onClick={() => void load()}>
          Повторить
        </button>
      </div>
    )
      ...
      {!opened ? (
              ...
              return (
                ...
                >
                  <div className="adm-pf-cover">
                    ...
                    >
                      <i className="bx bx-folder" style={{ fontSize: 52, opacity: 0.42, color: "var(--adm-active-color, #6366f1)" }} aria-hidden />
                    </div>
                  </div>
        ...
        <>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <button type="button" className="sp-btn sp-btn-ghost" style={{ fontSize: "0.8rem" }} onClick={() => setOpenedProjectId(null)}>
              <i className="bx bx-chevrons-left" style={{ marginRight: 4 }} />
              Все объекты
            </button>
            <span style={{ fontWeight: 600, fontSize: "0.9rem", color: "var(--adm-text)" }}>{opened.name}</span>
          </div>

          <p style={{ fontSize: "0.78rem", color: "var(--adm-muted)", margin: 0, lineHeight: 1.45 }}>
            Работы — портретные плитки, как у дизайнера. Ниже — материалы на всю папку и списки файлов по работам.
          </p>

          <div style={{ fontSize: "0.68rem", textTransform: "uppercase", letterSpacing: "0.04em", color: "var(--adm-muted)", marginBottom: 6 }}>
            Работы
          </div>
          {opened.cards.length === 0 ? (
            <div style={{ fontSize: "0.8rem", color: "var(--adm-muted)", fontStyle: "italic" }}>В этой папке нет работ</div>
          ) : (
            <div className="adm-pf-grid">
                  ...
                  <div key={card.id} className="adm-pf-cell">
                    <div className="adm-pf-cover">
                      {cover ? (
                        ...
                        >
                          <i className="bx bx-image" style={{ fontSize: 32, opacity: 0.35, color: "var(--adm-muted)" }} aria-hidden />
                        </div>
                      )}
                    </div>
                    <div className="adm-pf-shade" aria-hidden />
                    ...
                    <div className="adm-pf-foot">
                      <span className="adm-pf-badge">{pluralRu(matCount, "файл", "файла", "файлов")}</span>
                    </div>
                  </div>
                )
              })}
          ...
          {opened.attachments.length > 0 && (
            <div style={{ marginTop: 8 }}>
              <div style={{ fontSize: "0.68rem", textTransform: "uppercase", letterSpacing: "0.04em", color: "var(--adm-muted)", marginBottom: 8 }}>
                Материалы к проекту
              </div>
              {opened.attachments.map((a) => (
                <div key={a.id} style={rowStyle}>
                  <span style={{ minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    ...
                    {a.file.filename}
                    {a.file.mimeType ? (
                      <span style={{ color: "var(--adm-muted)", fontSize: "0.72rem", marginLeft: 6 }}>{a.file.mimeType}</span>
                    ) : null}
                  </span>
                  <button
                    type="button"
