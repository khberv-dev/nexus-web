        ...
        <div className="sp-card">
            ...
            <div className="sp-card-bd">
                <div className="sp-onb-summary">
                    ...
                    <div className="sp-onb-summary__meta">Пройдено
                        этапов: {doneCount}/{ONBOARDING_TABLE_STEP_TYPES.length}</div>
                </div>
            ...
            <Modal open={!!viewStep} onClose={() => setViewStep(null)} maxWidth={640}>
                <div className="sp-modal-body" style={{padding: "16px 18px"}}>
                    ...
                    {viewStep === "TEST" && (() => {
                        ...
                        const attempts = (() => {
                            if (!testStep?.comment) return [] as Array<{
                                level?: string;
                                percent?: number;
                                passed?: boolean;
                                finishedAt?: string
                            ...
                            try {
                                const json = JSON.parse(testStep.comment) as {
                                    attempts?: Array<{
                                        level?: string;
                                        percent?: number;
                                        passed?: boolean;
                        ...
                        return (
                            <div style={{display: "grid", gap: 8, fontSize: "0.8rem"}}>
                                ...
                                <div><span style={{color: "var(--adm-muted)"}}>Ответов сохранено:</span> {answersCount}
                                </div>
                                {progressData && (
                                    <div><span
                                        style={{color: "var(--adm-muted)"}}>Текущий прогресс:</span> {progressData.answeredCount}/{progressData.total},
                                        верных: {progressData.liveCorrect}</div>
                                )}
                                {parsed.meta && (
                                    <div><span
                                        style={{color: "var(--adm-muted)"}}>Последний результат:</span> {parsed.meta.correctCount} верных
                                        · {parsed.meta.percent}% · {parsed.meta.passed ? "сдано" : "не сдано"}</div>
                                )}
                        ...
                        return (
                            <div style={{display: "grid", gap: 10, fontSize: "0.8rem"}}>
                                <div style={{display: "flex", gap: 16, flexWrap: "wrap"}}>
                                    ...
                                    {finishedAt && <div><span
                                        style={{color: "var(--adm-muted)"}}>Дата:</span> {new Date(finishedAt).toLocaleString("ru-RU")}
                                    </div>}
                                </div>
                                ...
                                {ss && (
                                    <div style={{
                                        ...
                                        <div style={{color: "var(--adm-muted)", marginBottom: 6, fontWeight: 600}}>По
                                            разделам:
                                        </div>
                                ...
                                {answers && (
                                    <div style={{
                                        ...
                                        <div
                                            style={{color: "var(--adm-muted)", marginBottom: 6, fontWeight: 600}}>Ответы
                                            по вопросам:
                                        </div>
                                                ...
                                                <div key={idx} style={{
                                                    ...
                                                    <div style={{
                                                        ...
                                                        {!isCorrect && <span style={{
                                                            color: "var(--adm-muted)",
                                                            marginLeft: 8
                                                        }}>Верно: {q.options[q.correct]}</span>}
                                                    </div>
                                                </div>
                                            )
                                        })}
