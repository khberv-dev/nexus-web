...
export function SystemInfoCard({ specialist }: { specialist: RawSpecialist }) {
  ...
  const rows = [
    ...
    { label: "Регистрация", value: new Date(specialist.createdAt).toLocaleString("ru-RU"), icon: "bx-calendar", color: "#f59e0b" },
    ...(prof
      ? [
          ...
          {rows.map((item) => (
            <div key={item.label} className="sp-info-item">
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div className={`sp-info-value${item.value ? "" : " sp-info-value--empty"}`} title={typeof item.value === "string" ? item.value : undefined}>
                  {item.value || "—"}
                </div>
              </div>
            </div>
