    ...
    <div className="sp-card">
      ...
      <div className="sp-card-bd">
          ...
          <div className="sp-stars">
            {[1, 2, 3, 4, 5].map((star) => (
              <button
                ...
                style={{ opacity: isActiveSpec ? 1 : 0.45, cursor: isActiveSpec ? "pointer" : "not-allowed" }}
              >
                ★
              </button>
            ))}
        ...
        <div className="sp-landing-toggle" style={{ marginTop: 10 }}>
          <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: isActiveSpec ? "pointer" : "default", fontSize: "0.82rem" }}>
            <input
              type="checkbox"
          ...
          <p style={{ margin: "4px 0 0", fontSize: "0.7rem", color: "var(--adm-muted, #9ca3af)" }}>
            Управляется через <a href="/admin/landing" style={{ color: "var(--adm-active-color, #6366f1)" }}>модерацию сборок</a>. Ручное переключение перезаписывает статус.
          </p>
        </div>
      </div>
    </div>
