...
export function QuestionnaireCard({
  ...
  if (!Object.values(fd).some(Boolean)) {
    return (
      <div className="sp-warn"><i className="bx bx-info-circle" style={{ marginRight: 6 }} />Анкета не заполнена</div>
    )
  }
      ...
      <div className="sp-card-bd">
        <div className="sp-info-grid">
          {[
            ...
            { label: "Опыт", value: fd.experience ? `${fd.experience} лет` : "", icon: "bx-briefcase", color: "#f59e0b" },
            ...
            { label: "Специализация", value: fd.specialty || fd.specialization, icon: "bx-target-lock", color: "#f97316" },
            ...
            <div key={item.label} className="sp-info-item">
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div className={`sp-info-value${item.value && item.value !== "—" ? "" : " sp-info-value--empty"}`}>{item.value || "—"}</div>
              </div>
            </div>
          ))}
        </div>
        {fd.portfolio && (
          <div className="sp-info-item" style={{ marginTop: 10 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <a href={fd.portfolio} target="_blank" rel="noopener noreferrer" className="sp-info-link">{fd.portfolio}</a>
            </div>
          </div>
        )}
        {(taxLabel || fd.inn || fd.bankAccount) && (
          <div style={{ marginTop: 14, paddingTop: 12, borderTop: "1px solid var(--adm-sidebar-border, rgba(255,255,255,0.08))" }}>
            <div className="sp-info-label" style={{ marginBottom: 8 }}>Налоговый статус и реквизиты</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 10, fontSize: "0.82rem" }}>
              <div><span style={{ color: "var(--adm-muted)" }}>Статус: </span>{taxLabel || "—"}</div>
              {fd.taxStatus === "OOO" && <div style={{ gridColumn: "1 / -1" }}><span style={{ color: "var(--adm-muted)" }}>Наименование: </span>{fd.companyName || "—"}</div>}
              <div><span style={{ color: "var(--adm-muted)" }}>ИНН: </span>{fd.inn || "—"}</div>
              {fd.taxStatus === "IP" && <div><span style={{ color: "var(--adm-muted)" }}>ОГРНИП: </span>{fd.ogrnip || "—"}</div>}
              {fd.taxStatus === "OOO" && <div><span style={{ color: "var(--adm-muted)" }}>КПП: </span>{fd.kpp || "—"}</div>}
              {fd.taxStatus === "OOO" && <div><span style={{ color: "var(--adm-muted)" }}>ОГРН: </span>{fd.ogrn || "—"}</div>}
              {fd.ipName ? <div style={{ gridColumn: "1 / -1" }}><span style={{ color: "var(--adm-muted)" }}>ФИО (ИП): </span>{fd.ipName}</div> : null}
              {fd.taxStatus === "OOO" && <div style={{ gridColumn: "1 / -1" }}><span style={{ color: "var(--adm-muted)" }}>Юр. адрес: </span>{fd.legalAddress || "—"}</div>}
              ...
              {fd.taxStatus === "OOO" && <div><span style={{ color: "var(--adm-muted)" }}>Корр. счет: </span>{fd.corrAccount || "—"}</div>}
            </div>
          </div>
        )}
        {fd.about && (
