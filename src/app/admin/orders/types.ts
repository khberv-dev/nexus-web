...
export type StageStatus = "AWAITING_PAYMENT" | "BLOCKED" | "PENDING" | "UPLOADED" | "MOD_REVIEW" | "MOD_REVISION" | "CLIENT_REVIEW" | "CLIENT_REVISION" | "APPROVED" | "EXTRA_PAYMENT"
...
export interface SpecialistForAssignment {
  ...
  specialistProfile: { onboardingStatus: string; rating: number | null; formData: Record<string, string> | null } | null
  files: { id: string }[]
}
export type FileAudience = "DESIGNER" | "CLIENT" | "SHARED"
export interface StageFile { id: string; filename: string; uploadedAt?: string; audience?: FileAudience }
export interface StageReview { id: string; reviewerRole: "MODERATOR" | "CLIENT"; verdict: "APPROVED" | "REJECTED"; comment: string | null; createdAt: string }
...
export interface Order {
  id: string; title: string | null; price: number | null; status: OrderStatus; createdAt: string; updatedAt?: string
  briefData: Record<string, string> | null
  briefStep: number; briefHelpRequested: boolean
  briefVideoFile?: { id: string; s3Key: string; filename: string; mimeType: string | null; createdAt: string } | null
  client: User; specialist: User | null
  stages: Stage[]
