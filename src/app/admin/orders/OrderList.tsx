...
export function OrderList({ filtered, loading, selected, search, filter, onSelect, onSearch, onFilter }: Props) {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      document.getElementById(`admin-order-row-${selected}`)?.scrollIntoView({ block: "nearest", behavior: "smooth" })
    })
  }, [loading, selected])
  ...
  return (
    <aside className="sp-list">
      ...
      <div className="sp-search">
        ...
        <input type="text" className="sp-search-input" placeholder="Поиск…" value={search} onChange={e => onSearch(e.target.value)} />
      </div>
      <div className="sp-filters">
        {FILTERS.map(f => (
          <button key={f.value} className={`sp-filter-btn${filter === f.value ? " sp-filter-btn--on" : ""}`} onClick={() => onFilter(f.value)}>{f.label}</button>
        ))}
      </div>
          ...
          >
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 4 }}>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: "0.82rem", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{title}</div>
                <div style={{ fontSize: "0.72rem", color: "var(--adm-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{o.client.name ?? o.client.email}</div>
                {hint.text ? (
            ...
            {showMetaRow && (
              <div style={{ display: "flex", gap: 4, marginTop: 4, flexWrap: "wrap" }}>
                ...
                {o.briefHelpRequested && <span className="sp-badge sp-badge--danger" style={{ fontSize: "0.6rem" }}><i className="bx bx-support" style={{ marginRight: 3 }} />помощь</span>}
                {modStages.length > 0 && <span className="sp-badge sp-badge--danger" style={{ fontSize: "0.6rem" }}><i className="bx bx-time" style={{ marginRight: 3 }} />модерация</span>}
                {needsAssign && <span className="sp-badge sp-badge--warn" style={{ fontSize: "0.6rem" }}><i className="bx bx-user-plus" style={{ marginRight: 3 }} />специалист</span>}
              </div>
            )}
          </div>
        )
