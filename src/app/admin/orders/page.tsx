...
export default function OrdersAdminPage() {
  ...
  const [clientRevModal, setClientRevModal] = useState<{ stageId: string; stageName: string; action: "accept" | "reject" } | null>(null)
  ...
  const assign = async (orderId: string) => {
    ...
    await fetch(`/api/admin/orders/${orderId}/assign`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ specialistId }) })
    await load(); setAssigning(null)
  }

  const reviewStage = async (stageId: string, action: "modApprove" | "modRevision", stageName: string) => {
    if (action === "modRevision") { setRevisionComment(""); setRevisionModal({ stageId, stageName }); return }
    setActing(stageId)
    await fetch(`/api/admin/stages/${stageId}/review`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) })
  ...
  const submitRevision = async () => {
    ...
    setActing(revisionModal.stageId); setRevisionModal(null)
    await fetch(`/api/admin/stages/${revisionModal.stageId}/review`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "modRevision", comment: revisionComment || undefined }) })
    setRevisionComment(""); await load(); setActing(null)
  }

  const submitExtra = async () => {
    if (!extraForm || !extraAmount) return
    setActing(extraForm.stageId); setExtraForm(null)
    await fetch(`/api/admin/payments/${extraForm.stageId}/extra`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ amount: Math.round(Number(extraAmount) * 100), reason: extraReason || "Дополнительные правки" }) })
    setExtraAmount(""); setExtraReason(""); await load(); setActing(null)
  }
  ...
  const briefApprove = async (orderId: string) => {
    ...
    await fetch(`/api/admin/orders/${orderId}/brief`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "approve" }) })
    await load(); setActing(null)
  }
  ...
  const submitBriefReject = async () => {
    ...
    await fetch(`/api/admin/orders/${briefRejectModal.orderId}/brief`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "reject", comment: briefRejectComment }) })
    setBriefRejectComment("")
    await load()
    setActing(null)
  }
    ...
    <AdminLayout noPadding>
      ...
      <Modal open={!!revisionModal} onClose={() => setRevisionModal(null)} maxWidth={480}>
        <div style={{ padding: "24px 24px 20px" }}>
          ...
          <p style={{ color: "var(--adm-muted)", fontSize: "0.875rem", margin: "0 0 16px" }}>{revisionModal?.stageName}</p>
          <textarea className="sp-textarea" rows={4} placeholder="Замечания…" value={revisionComment} onChange={e => setRevisionComment(e.target.value)} autoFocus />
      ...
      <Modal open={!!clientRevModal} onClose={() => setClientRevModal(null)} maxWidth={480}>
        <div style={{ padding: "24px 24px 20px" }}>
          ...
          <p style={{ color: "var(--adm-muted)", fontSize: "0.875rem", margin: "0 0 16px" }}>{clientRevModal?.stageName}</p>
          <textarea
      ...
      <Modal open={!!briefRejectModal} onClose={() => setBriefRejectModal(null)} maxWidth={480}>
        <div style={{ padding: "24px 24px 20px" }}>
          ...
          <p style={{ color: "var(--adm-muted)", fontSize: "0.875rem", margin: "0 0 16px" }}>Укажите причину возврата</p>
          <textarea className="sp-textarea" rows={4} placeholder="Причина возврата…" value={briefRejectComment} onChange={e => setBriefRejectComment(e.target.value)} autoFocus />
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            <button className="sp-btn sp-btn-ghost" onClick={() => setBriefRejectModal(null)}>Отмена</button>
            <button className="sp-btn sp-btn-danger" onClick={submitBriefReject} disabled={!briefRejectComment.trim()}>Вернуть</button>
          </div>
        </div>
      </Modal>
      ...
      <Modal open={!!extraForm} onClose={() => setExtraForm(null)} maxWidth={480}>
        <div style={{ padding: "24px 24px 20px" }}>
          ...
          <p style={{ color: "var(--adm-muted)", fontSize: "0.875rem", margin: "0 0 16px" }}>{extraForm?.stageName}</p>
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: "block", fontSize: "0.75rem", color: "var(--adm-muted)", marginBottom: 4 }}>Сумма (руб.)</label>
            <input type="number" className="sp-input" value={extraAmount} onChange={e => setExtraAmount(e.target.value)} placeholder="5000" />
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: "block", fontSize: "0.75rem", color: "var(--adm-muted)", marginBottom: 4 }}>Причина</label>
            <input type="text" className="sp-input" value={extraReason} onChange={e => setExtraReason(e.target.value)} placeholder="Дополнительные правки" />
          </div>
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            <button className="sp-btn sp-btn-ghost" onClick={() => setExtraForm(null)}>Отмена</button>
            <button className="sp-btn sp-btn-primary" onClick={submitExtra} disabled={!extraAmount}>Выставить счет</button>
          </div>
        </div>
      </Modal>
      ...
      <div className="sp-wrap">
        ...
        <OrderDetail
          ...
          onExtraPayment={(sid, name) => { setExtraAmount(""); setExtraReason(""); setExtraForm({ stageId: sid, stageName: name }) }}
          onChangeStatus={changeStatus}
          onBriefApprove={briefApprove} onBriefReject={briefReject}
          onBriefSaved={load}
          onResolveHelp={async (orderId) => {
            setActing("resolve-help")
            await fetch(`/api/admin/orders/${orderId}/brief`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "resolve_help" }) })
            await load()
            setActing(null)
          }}
