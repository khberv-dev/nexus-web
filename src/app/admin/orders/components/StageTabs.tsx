      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "nowrap", minWidth: "max-content" }}>
        {orderedStages.map((s) => {
          ...
          return (
            ...
            >
              {STAGE_LABEL[s.type]}
              {hasMod ? <span className="sp-badge sp-badge--danger" style={{ fontSize: "0.6rem" }}>!</span> : null}
              {!hasMod && actNeedsAdmin ? (
                <span className="sp-badge sp-badge--warn" style={{ fontSize: "0.6rem" }} title="Акт ждёт действия">
                  акт
                </span>
              ) : null}
            </button>
