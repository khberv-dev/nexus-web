            ...
            {[
              ...
              return sum > 0 ? (
                <div key={p.s}>
                  ...
                  <span style={{ fontWeight: 600, color: p.c }}>{(sum / 100).toLocaleString("ru-RU")} руб.</span>
                </div>
              ) : null
            })}
          </div>
      ...
      {order.status === "BRIEF_REVIEW" && (
        <div className="sp-brief-actions">
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => onBriefApprove(order.id)} disabled={acting !== null} className="sp-btn sp-btn-success">
              {acting === "brief-approve" ? "…" : "Одобрить бриф"}
            </button>
            <button onClick={() => onBriefReject(order.id)} disabled={acting !== null} className="sp-btn sp-btn-danger">
              Вернуть бриф
            </button>
          </div>
        </div>
