          ...
          <div key={s.label} className="col-md-4">
            <div className="card">
              <div className="card-body">
                ...
                <h4 className="mb-0 fw-semibold">{fmt(s.value)} <small className="text-muted fs-6">руб.</small></h4>
              </div>
            </div>
          </div>
        ))}
              ...
              <tbody>
                {filtered.map(p => (
                  <tr key={p.id}>
                    ...
                    <td className="text-muted" style={{ fontSize: "0.875rem" }}>{p.order.client.email}</td>
                    <td className="text-muted" style={{ fontSize: "0.875rem" }}>
                      {p.order.specialist?.email ?? <span className="text-danger">—</span>}
                    </td>
                    <td><StatusBadge variant={STATUS_VARIANT[p.status]} label={STATUS_LABEL[p.status]} /></td>
