                            ...
                            fw.frameworkContractStatus !== "SIGNED_BY_ADMIN" && (
                                <div style={{marginBottom: 12}}>
                                    <button
                                        ...
                                        onClick={async () => {
                                            const ok = await confirmDialog({
                                                title: "Зафиксировать подписание договора?",
                                                description: "Черновики и брифы этого заказчика с заполненными данными перейдут в статус «Активен» — можно назначать специалистов. Это действие нельзя отменить.",
                                                variant: "destructive",
                                            })
                                            if (!ok) return
